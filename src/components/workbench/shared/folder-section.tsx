...
export function FolderSection({
  ...
  // 거치지 않는다. 매 pointermove 의 setState 는 섹션 전체(폴더 칩 수십 장 +
  ...
  const beginListHeightResize = (e: React.PointerEvent) => {
    ...
    // [data-folder-list-pane] (한 화면에 FolderSection 이 여럿이어도 자기 창만).
    ...
    const onUp = () => {
      ...
      try {
        window.localStorage.setItem(LIST_HEIGHT_KEY, String(latest));
      } catch {
        /* ignore */
      }
      try {
        ...
        {!collapsed && (!enableFolderControls || viewMode === "grid" || hideHeader) ? (
          ...
          <div
            data-folder-list-pane={
              enableFolderControls || resizableGrid ? "" : undefined
            }
            style={
              enableFolderControls || resizableGrid
                ? { height: listHeight }
