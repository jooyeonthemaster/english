...
const STATS: ReadonlyArray<{
  ...
  { key: "total", label: "총 전환", icon: UserPlus, pick: (s) => s.totalSignups },
  { key: "granted", label: "지급 완료", icon: CheckCircle2, tone: "emerald", pick: (s) => s.granted },
          ...
          >
            ...
            <div className="cursor-pointer rounded-xl outline-none transition-shadow hover:shadow-md focus-visible:ring-2 focus-visible:ring-blue-500/30">
              <StatCard
                size="sm"
                label={s.label}
                value={s.pick(stats).toLocaleString("ko-KR")}
                icon={s.icon}
                valueTone={s.tone}
              />
            </div>
  ...
  return (
    ...
    >
      <Tr clickable>
        ...
        <Td align="right">+{(row.referrerReward + row.referredReward).toLocaleString("ko-KR")}</Td>
        ...
        <Td align="right">
          ...
          <AdminDialog
            ...
            size="sm"
            title="추천 보상 회수"
            description={`${row.referrerAcademyName} → ${row.referredAcademyName} · 지급된 ${(row.referrerReward + row.referredReward).toLocaleString("ko-KR")} 크레딧을 되돌립니다.`}
            footer={
              <>
