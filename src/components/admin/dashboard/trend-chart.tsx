 ...
 * 막대의 <title> 은 위에 덮인 상호작용 열에 가려지므로, 같은 「환불」 자구를 열 라벨에도 싣는다.
 */
export function TrendChart({ data }: { data: DashboardTrendPoint[] }) {
  ...
  const padX = 8;
  const padTop = 16;
  const padBottom = 30; // 0선 아래 음수(환불) 막대 자리 + 날짜 라벨
        ...
        >
          {data.map((d) => (
            <DashboardDetailBlock
              ...
              detailKey="trend-day"
              param={d.dateKey}
              label={`${d.date} ${d.revenue < 0 ? "환불" : "매출"} ${formatCurrency(d.revenue)} · 가입 ${d.signups}`}
              side="top"
