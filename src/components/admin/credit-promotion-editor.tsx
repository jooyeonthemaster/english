...
// toDatetimeLocal 등)은 -parts/form-utils 에 있고 여기서 그대로 re-export 한다.
// 예외는 formatDate — 표시 시각은 KST 고정이어야 해서 이 파일에서 다시 정의한다(아래).
// ============================================================================
...
/**
 * 관리자 결제·프로모션 화면 공용 시각 표기 — 「26. 09. 17. 22:46」(KST 고정, 연도 포함).
 *
 * form-utils 의 동명 함수는 timeZone·year 가 없어 실행 환경의 시간대로 렌더된다
 * (Vercel 런타임은 UTC → SSR 결과가 KST 보다 9시간 이르다). 이 화면군의 소비자
 * (상품·프로모션 표, 결제 상세, 지표 카드)는 모두 이 경로로 formatDate 를 가져가므로
 * re-export 목록에서 formatDate 만 빼고 여기서 KST 포매터로 다시 정의한다. 스펙 §9.2 I1.
 */
