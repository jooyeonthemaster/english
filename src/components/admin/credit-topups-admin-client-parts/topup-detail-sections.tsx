      ...
      {events.length === 0 ? (
        ...
        events.map((event) => (
          <div key={event.id} className="rounded-lg border border-gray-100 px-3 py-2">
            ...
            <div className="mt-1 text-[11px] text-gray-400">{formatDate(event.receivedAt)}</div>
            {event.errorMessage && (
      ...
      {transactions.length === 0 ? (
        ...
        transactions.map((tx) => (
          <div key={tx.id} className="rounded-lg border border-gray-100 px-3 py-2">
            ...
            <div className="mt-1 text-[11px] text-gray-400">{formatDate(tx.createdAt)}</div>
          </div>
        ))
      )}
    </DetailSection>
          ...
          <div className={cn("space-y-2 transition-opacity", loading && "opacity-60")}>
            {items.map((tx) => (
              <div key={tx.id} className="rounded-lg border border-gray-100 px-3 py-2">
                ...
                {tx.description && (
                  <div className="mt-0.5 text-[11px] text-gray-400">{formatDate(tx.createdAt)}</div>
                )}
              </div>
            ))}
          </div>
