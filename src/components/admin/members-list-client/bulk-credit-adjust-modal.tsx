...
import { Plus, Minus, Loader2, Coins, Users } from "lucide-react";
...
const FORM_ID = "bulk-credit-adjust-form";
...
export function BulkCreditAdjustModal({
  ...
  return (
    <AdminDialog
      ...
      description={
        <>
          선택한 <span className="font-medium text-gray-700">{count}명</span>의 회원에게 동일한
          크레딧을 지급하거나 회수합니다. 모든 조정은 회원별 거래 내역에 기록됩니다.
        </>
      }
      footer={
        <>
          ...
          <Button
            ...
            size="sm"
            disabled={!canSubmit}
            className={cn("min-w-[90px]", direction === "deduct" && "bg-rose-600 hover:bg-rose-700")}
          >
            {isPending ? (
              <>
    ...
    >
      <form id={FORM_ID} onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-blue-100 bg-blue-50/60 px-3 py-2">
          <Users className="size-4 shrink-0 text-blue-600" strokeWidth={2} aria-hidden />
          <span className="text-[12px] text-blue-800">
            대상 회원 <span className="font-semibold tabular-nums">{count}명</span>
          </span>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder="0"
              className="h-10 pl-9! pr-12! text-[13px] tabular-nums"
          ...
          <div className="flex items-center justify-between text-[11px]">
            {tooLarge ? (
              <span className="text-rose-600">최대 {MAX_AMOUNT.toLocaleString("ko-KR")} 크레딧</span>
            ) : (
              <span className="text-gray-400">1인당 최대 {MAX_AMOUNT.toLocaleString("ko-KR")}</span>
            )}
            {numericAmount > 0 && (
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <span className="text-[11px] font-medium text-gray-400">빠른 추가</span>
        ...
        {direction === "deduct" && (
          <p className="rounded-lg border border-amber-100 bg-amber-50 px-3 py-2 text-[11px] text-amber-700">
            차감 시 잔고가 부족한 회원은 자동으로 건너뛰며, 완료 후 성공·실패 건수를
            알려드립니다.
          </p>
        )}
...
function DirectionButton({
  ...
  return (
    <Button
      ...
      className={cn(
        "h-10 text-[13px]",
        active && tone === "positive" && "border-blue-600 bg-blue-600 text-white hover:bg-blue-700 hover:text-white",
        active && tone === "negative" && "border-rose-600 bg-rose-600 text-white hover:bg-rose-700 hover:text-white",
