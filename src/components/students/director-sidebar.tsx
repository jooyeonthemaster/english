...
export function DirectorSidebar({ academyName }: DirectorSidebarProps) {
  const [collapsed, setCollapsed] = useState(false);
  const pathname = usePathname();
  const navGroups = getNavGroups("/director");

  function isActive(href: string): boolean {
    if (href === "/director") {
      return pathname === "/director" || pathname === "/director/";
    }
  ...
  return (
    ...
    >
      ...
      {/* Navigation */}
      <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-3" aria-label="원장 메뉴">
