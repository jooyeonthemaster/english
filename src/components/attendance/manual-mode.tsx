...
export function ManualMode({ academyId, classes }: ManualModeProps) {
  ...
  const statusColor = (status: AttendanceStatus) =>
    ATTENDANCE_STATUSES.find((s) => s.value === status)?.color || "bg-gray-300";
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-card">
        ...
        <div className="flex items-center gap-1 ml-auto">
          <span className="text-xs text-gray-400 mr-1">일괄:</span>
          {ATTENDANCE_STATUSES.map((s) => (
            <button
              key={s.value}
              onClick={() => handleBulkMark(s.value)}
              className="h-7 px-2.5 rounded-md text-xs font-medium text-gray-600 hover:bg-gray-100 transition-colors"
              title={`전체 ${s.label}`}
            >
              {s.label}
            </button>
                        ...
                        {ATTENDANCE_STATUSES.map((s) => (
                          ...
                          >
                            <button
                              ...
                              className={cn(
                                "h-8 w-8 rounded-full border-2 transition-all duration-200 mx-auto flex items-center justify-center",
