...
//
//  무엇이 매출인가의 **단일 진실원은 src/lib/admin-revenue.ts** 다
//  (계약: docs/analytics/analytics-spec.md I9). 금액·매출 인식 시각은 조건을
//  베껴 쓰지 않고 그 파일의 헬퍼를 **호출해서** 만든다 — 관리자 콘솔과 이
//  피드가 같은 줄에서 같은 숫자를 내야 하기 때문이다:
//    · 크레딧 충전  topUpAmount()    = paidAmount ?? price
//                   topUpPaidAt()    = paidAt ?? completedAt
//                   topUpRefundedAt()= cancelledAt ?? updatedAt
//    · 구독 결제    status = PAID, subscriptionPaidAt() = paidAt ?? completedAt
//    · 무통장 수기  status = MANUAL_GRANT, manualGrantAt() = occurredAt ?? receivedAt
//
//  ⚠️ 아직 하나 남은 차이 — **환불된 충전(status=REFUNDED)**.
//     admin-revenue(I9)는 「결제일에는 매출이었다」를 지키려고 REFUNDED 도
//     결제일 gross 에 넣고 환불일에 빼지만(소급 삭제 금지), 이 피드는 그 줄을
//     `revenue:false, excluded:"refunded"` 로 보낸다. 두 정의는 환불이 결제와
//     **같은 달**에 일어나면 같은 값을, 달을 넘기면 다른 값을 낸다.
//     26-09-18 실DB 대조: REFUNDED 2건(6/9→6/10, 7/15→7/15) 둘 다 같은 달
//     → 월별·최근 30일·이번 달 전 구간 차이 0원(합계 1,770,000원 일치).
//     고치려면 revenue 의 뜻이 바뀌므로 FEED_VERSION 을 3으로 올리고 ERP
//     저장소의 contract.ts 를 함께 배포해야 한다 — 한쪽만 바꾸면 ERP 가
//     동기화를 멈춘다. 그래서 여기서는 손대지 않는다.
//     (부분취소는 실DB 0건. 발생하면 amount.cancelled 로 차감돼 피드가 맞다.)
//
//  부르는 법:
...
// 매출 정의 단일 진실원 — 조건·금액·시각을 여기서 베끼지 말고 호출할 것(I9).
import {
  manualGrantAt,
  subscriptionPaidAt,
  topUpAmount,
...
async function loadSales(
  ...
  for (const t of topUps) {
    // 금액·시각은 admin-revenue.ts 헬퍼가 만든다(I9) — 관리자 콘솔과 같은 줄에서
    // 같은 숫자가 나와야 한다.
    const gross = topUpAmount(t);
    const cancelled = cancelledAmountOf(t.paymentPayload);
    // REFUNDED 는 전액 취소다(portone-credit-topups.ts 의 nextStatus). 옛 줄이라
    // 응답 원본이 비어 amount.cancelled 를 못 읽어도 **환불액 0 으로 보내지
    // 않는다** — 그러면 ERP 에 「환불됐는데 돌려준 돈은 0」인 줄이 남는다.
    const refund = t.status === "REFUNDED" && cancelled === 0 ? gross : cancelled;
    ...
    const row: SmoatSaleRow = {
      ...
      amount: gross,
      refundAmount: refund,
      // 환불이 실제로 있었을 때만 채운다(없는 줄에 updatedAt 을 흘리면 ERP 가
      // 환불로 읽는다). 있으면 cancelledAt 이 비어도 updatedAt 으로 메운다.
      refundedAt: refund > 0 ? iso(topUpRefundedAt(t)) : null,
      ...
      row.excluded =
        t.status === "REFUNDED"
          ...
          : t.status === "CANCELLED"
            ...
            : t.status === "FAILED"
              ? "failed"
              : // COMPLETED 인데 여기 온 것은 전액이 취소된 것뿐이다 (paidAt 이
                // 없는 COMPLETED 는 실DB 0건). 「pending」으로 보내면 ERP 가
                // 결제 대기로 읽는다.
                refund >= gross && gross > 0
                ? "refunded"
                : "pending";
    }
  ...
  for (const s of subscriptions) {
    ...
    const row: SmoatSaleRow = {
      ...
      amount: gross,
      refundAmount: refund,
      refundedAt: refund > 0 ? iso(s.cancelledAt ?? s.updatedAt) : null,
      paymentMethod: "CARD",
  ...
  const depositRows: SmoatSaleRow[] = deposits.map((d) => {
    // 무통장 수기 지급의 매출 인식 시각도 admin-revenue 헬퍼(I9)로 맞춘다.
    const at = manualGrantAt(d);
