      ...
      <div className="col-span-2 space-y-4">
        <div className="bg-white rounded-2xl border p-5 space-y-5">
          <div className="flex items-center justify-between">
            ...
            >
              {autoFill
                ? `자동 채우기 ON (${Object.values(typeCounts).reduce((a, b) => a + b, 0)}개)`
                : "자동 채우기"}
            </button>
          </div>
          ...
          {CATEGORY_GROUPS.map((group) => {
            const groupTotal = group.items.reduce(
              (sum, item) => sum + (typeCounts[item.id] || 0),
              0,
            );
            return (
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            !autoFill &&
                            canAddMore &&
                            onTypeCountChange(item.id, count + 1)
                          }
                        ...
                        {active && (
                          <div className="flex items-center gap-0.5 pr-1 border-l border-slate-200">
                            {!autoFill && (
                              <button
                                onClick={() =>
                                  onTypeCountChange(item.id, count - 1)
                                }
                                className="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-blue-600 text-[14px] font-bold"
                              >
                                -
                              </button>
                            )}
                            <span
                              className={`text-center text-[12px] font-bold text-slate-800 ${autoFill ? "px-1.5" : "w-5"}`}
                            >
                              {count}
                            </span>
                            {!autoFill && (
                              <button
                                ...
                                disabled={!canAddMore}
                                className="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-blue-600 text-[14px] font-bold disabled:text-slate-300 disabled:cursor-not-allowed"
                              >
                                +
                              </button>
                            )}
