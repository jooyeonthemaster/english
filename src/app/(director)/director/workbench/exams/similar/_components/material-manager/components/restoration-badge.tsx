...
export function RestorationBadge({ status }: { status: string }) {
  if (status === "PENDING") {
    return (
      <span className="relative inline-flex items-center gap-1 rounded-full bg-slate-100 px-1.5 py-0 text-[9.5px] font-bold text-slate-600">
        <span
          aria-hidden="true"
  ...
  if (status === "FAILED") {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-b from-red-50 to-red-100/70 px-1.5 py-0 text-[9.5px] font-bold text-red-700 ring-1 ring-red-200/70">
