...
export function PromptSection({
  ...
  return (
    <div
      data-generate-tour="prompt-section"
      className={fill ? "flex flex-1 min-h-0 flex-col gap-2.5" : "space-y-2.5"}
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            ...
            <ChevronDown
              className={`w-3 h-3 transition-transform duration-200 ${showSavedPrompts ? "rotate-180" : ""}`}
            />
          </button>
        </div>
      </div>
      ...
      {showSaveInput && (
        <div className="flex items-center gap-2 p-2 rounded-xl bg-teal-50/80 border border-teal-200/60">
          <input
            ...
            className="flex-1 h-8 px-2.5 text-[12px] rounded-lg border border-teal-200 bg-white outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-400/10"
            autoFocus
            onKeyDown={(e) => {
      ...
      {showSavedPrompts && (
        <div className="rounded-xl border border-slate-200 overflow-hidden bg-white">
          {savedPrompts.length === 0 ? (
            <div className="px-4 py-5 text-center text-[12px] text-slate-400 font-medium">
              저장된 지시사항이 없습니다
            </div>
          ) : (
            <div className="divide-y divide-slate-100 max-h-[160px] overflow-y-auto">
              {savedPrompts.map((sp) => (
                <div
                  key={sp.id}
                  className="flex items-center gap-2 px-3 py-2.5 hover:bg-slate-50 group transition-colors"
                >
                  {editingPromptId === sp.id ? (
                    <div className="flex items-center gap-1.5 flex-1">
                      <input
                        ...
                        className="flex-1 h-7 px-2 text-[12px] rounded-md border border-slate-200 outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-400/10"
                        autoFocus
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            updateCustomPrompt(sp.id, {
                              name: editingName,
                            }).then(() => {
                      ...
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center text-slate-500 hover:bg-blue-50 hover:text-blue-600 rounded transition-colors"
                      >
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                        <button
                          ...
                          className="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-slate-600 rounded transition-colors"
                        >
                          <Pencil className="w-3 h-3" />
                        </button>
                        <button
                          onClick={() => {
                            if (confirm(`"${sp.name}" 삭제?`))
