    ...
    return startAdaptivePoll({
      ...
      run: async (signal) => {
        try {
          ...
          // 진행 중(PENDING) 잡이 있는 동안은 서명에 시각을 섞어 백오프를 막는다
          // — 생성 중에는 status 가 한동안 안 변해 idleMs(5분)까지 늘어지고,
          // 그만큼 완료 반영이 늦어지기 때문. 잡이 없으면 기존 백오프 그대로.
          const base = jobs
            .map((j) => `${j.id}:${j.status}:${j.successCount}`)
            .join("|");
          const anyPending = jobs.some((j) => j.status === "PENDING");
          return anyPending ? `${base}|t${Date.now()}` : base;
        } catch {
