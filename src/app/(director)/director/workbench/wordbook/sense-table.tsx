...
export function SenseTable({
  ...
  // 2026-08-20 누적 개편 전에는 마키가 **담기 전용**이어야 했다 — 그때는 비추가
  // 드래그가 「선택 교체」라, 그대로 쓰면 이전에 담아 둔 장바구니가 통째로
  // 지워졌기 때문이다. 지금은 DragSelect 가 누적을 보장하므로 그 위험이 없고,
  // 빼기는 아래 해제 드래그(담긴 행에서 시작) · + 클릭 · 담은 단어 패널로 한다.
  ...
  // · 담기 드래그(빈 곳/미담김 행에서 시작): next 는 기존 장바구니 ∪ 히트라
  //   새 항목만 골라 담는다(다른 렌즈에서 담아 둔 것은 next 에 그대로 실려 온다).
