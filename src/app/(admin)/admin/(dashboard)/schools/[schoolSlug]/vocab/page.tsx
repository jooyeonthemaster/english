...
import { deleteVocabList } from "@/actions/vocab";
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <nav className="flex items-center gap-1.5 text-[13px] text-[#8B95A1]">
        ...
        <ChevronRight className="size-3.5" />
        <span className="font-medium text-[#191F28]">단어장 관리</span>
      </nav>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-[14px] text-[#8B95A1]">
              {school.name} - 총 {vocabLists.length}개 단어장
            </p>
          </div>
        </div>
        <Link href={`/admin/schools/${school.slug}/vocab/create`}>
          <Button className="gap-2 bg-[#3182F6] text-white hover:bg-[#1B64DA]">
            <Plus className="size-4" />새 단어장 등록
          </Button>
        </Link>
      </div>
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      ...
                      >
                        {gradeLabelMap[list.grade] || `${list.grade}학년`}
                      </Badge>
                      <Badge
                        variant="secondary"
                        className="bg-[#F7F8FA] text-[#4E5968] hover:bg-[#F7F8FA]"
                      >
                        {semesterLabelMap[list.semester] || list.semester}
                      </Badge>
                    </div>
                  </TableCell>
                      ...
                      <Dialog>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              &quot;{list.title}&quot; 단어장을 삭제하시겠습니까?
                              포함된 모든 단어도 함께 삭제됩니다. 이 작업은
                              되돌릴 수 없습니다.
                            </DialogDescription>
                          </DialogHeader>
                          <DialogFooter>
