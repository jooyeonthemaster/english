...
// 유료 크레딧 소멸시효 정책 시행일 (사전 공지 후 전 계정 적용).
const CREDIT_EXPIRY_EFFECTIVE_DATE = "2026년 7월 3일";
...
const TERMS_SECTIONS = [
  {
    ...
    body: [
      ...
      "SMOAT는 영어학원 운영, 학습 데이터 관리, AI 기반 문제 생성·학습지 생성·텍스트 추출 등 디지털 기능을 제공하는 SaaS 서비스입니다.",
    ],
  },
  {
    title: "제2조 용어의 정의",
    body: [
      ...
      "AI 기능은 문제 생성, 자동 출제, 학습지 생성, 텍스트 추출, 해설 생성, 문제 수정, AI 튜터링 등 크레딧을 차감하여 제공되는 디지털 기능을 말합니다.",
      "크레딧 소멸시효(유효기간)는 지급된 크레딧을 유효하게 사용할 수 있는 기간으로, 상품별로 정해지며 소멸 예정일이 지나면 잔여 크레딧이 소멸되는 기준을 말합니다.",
      ...(BANK_DEPOSIT_ENABLED
        ? [
            "무통장입금(계좌이체)은 회원이 회사가 지정한 입금 계좌로 직접 입금하고, 회사가 입금자명과 입금 금액을 대조하여 입금 사실을 확인한 뒤 크레딧을 지급하는 결제 방식을 말합니다. 무통장입금은 포트원 및 PG사를 거치지 않습니다.",
          ]
        : []),
  ...
  },
  {
    title: "제6조 크레딧 사용 순서",
    body: [
      "크레딧 사용 및 환불 산정은 무료·보너스 크레딧, 유료 크레딧 순서로 차감된 것으로 봅니다.",
      "위 사용 순서는 미사용 유료 크레딧을 명확히 산정하고 회원의 환불 가능 범위를 보호하기 위한 기준입니다.",
      "별도 계약, 프로모션, 이벤트에서 다른 사용 순서를 명시한 경우 해당 조건이 우선 적용될 수 있습니다.",
    ],
  },
  {
    title: "제7조 크레딧 소멸시효(유효기간)",
    body: [
      "유료 크레딧에는 상품별로 정해진 소멸시효(유효기간)가 적용되며, 각 상품의 소멸시효는 결제 화면 및 상품 정보에 결제일 기준 일수로 표시됩니다.",
      "크레딧은 상품별로 유효일수가 다를 수 있으며, 상위 상품일수록 더 긴 소멸시효가 부여될 수 있습니다.",
      "회원이 크레딧을 구매하거나 회사가 유효기간이 정해진 크레딧을 지급하는 경우, 계정의 소멸 예정일은 '기존 잔여 기간 + 새로 지급되는 유효기간'으로 갱신됩니다. 이때 소멸 예정일은 단축되지 않고 연장되며, 계산은 24시간(만 하루)을 기준으로 합니다.",
      "이벤트, 프로모션, 보너스 등으로 지급되는 크레딧은 별도의 유효기간을 새로 부여하지 않고 계정이 보유한 기존 소멸 예정일을 그대로 따르며, 기존 소멸 예정일이 없는 경우에는 별도 고지 전까지 유효기간이 적용되지 않을 수 있습니다.",
      "크레딧을 사용(차감)하더라도 소멸 예정일은 변경되지 않으며, 소멸 예정일은 구매 또는 지급으로만 갱신됩니다.",
      "소멸 예정일이 도래하면 계정에 남아 있는 크레딧은 유료·무료 구분 없이 전액 소멸되며, 소멸된 크레딧은 복구되지 않고 환불 대상에 포함되지 않습니다.",
      "회사는 부정 사용 확인, 정책 위반, 오지급 정정 등 정당한 사유가 있는 경우 회원의 크레딧 소멸 예정일을 조정하거나 잔여 크레딧을 소멸시킬 수 있으며, 그 사유와 내역은 거래 내역에 기록됩니다.",
      `본 소멸시효 정책은 ${CREDIT_EXPIRY_EFFECTIVE_DATE}부터 시행되며, 시행일 이후 구매·지급되는 크레딧에 적용됩니다.`,
      "시행일 현재 보유 중인 크레딧에 대해서는 회사가 계정별 이용 현황 등을 고려하여 잔여 크레딧의 소멸 예정일을 부여하거나 잔여 크레딧을 조정·소멸시킬 수 있으며, 구체적인 처리 대상과 기준은 시행일 전 서비스 화면 및 공지사항을 통해 안내합니다.",
      "회사가 소멸시효 기준을 변경하는 경우 적용 대상, 시행일, 잔여 크레딧 처리 기준을 사전에 고지합니다.",
    ],
  },
  ...(SUBSCRIPTION_BILLING_ENABLED
    ? [
    ...
    : []),
  {
    ...
    body: [
      ...
      ...(BANK_DEPOSIT_ENABLED
        ? [
            "무통장입금(계좌이체)은 포트원 및 PG사를 거치지 않고, 회원이 회사 지정 계좌로 입금한 뒤 회사가 입금자명과 입금 금액을 대조하여 입금이 확인되면 크레딧이 지급됩니다. 입금 전에는 크레딧이 지급되지 않으며, 입금자명 또는 금액이 일치하지 않거나 안내된 시간 내 입금되지 않은 경우 자동 확인이 지연되어 별도 확인이 필요할 수 있습니다.",
            "무통장입금은 현금성 입금 방식으로 원 결제수단 취소가 적용되지 않으며, 환불이 필요한 경우 회원 명의 계좌로의 계좌 환불 등 별도 크레딧 환불 정책에 따라 처리됩니다.",
          ]
        : []),
      ...(SUBSCRIPTION_BILLING_ENABLED
        ? [
      ...
      ...(SUBSCRIPTION_BILLING_ENABLED
        ...
        : [
            "회원은 크레딧 구매 전 상품명, 가격, 지급 크레딧, 기능별 차감 기준, 환불 정책을 확인해야 합니다.",
          ]),
    ],
    link: { href: "/refund-policy", label: "환불 정책 보기" },
  },
  {
    title: `${SUBSCRIPTION_BILLING_ENABLED ? "제10조" : "제9조"} 계정 해지 시 잔여 크레딧 처리`,
    body: [
  ...
  {
    title: `${SUBSCRIPTION_BILLING_ENABLED ? "제11조" : "제10조"} 금지행위 및 이용 제한`,
    body: [
  ...
  {
    title: `${SUBSCRIPTION_BILLING_ENABLED ? "제12조" : "제11조"} AI 결과물 및 데이터`,
    body: [
  ...
  {
    title: `${SUBSCRIPTION_BILLING_ENABLED ? "제13조" : "제12조"} 개인정보 보호`,
    body: [
  ...
  {
    title: `${SUBSCRIPTION_BILLING_ENABLED ? "제14조" : "제13조"} 책임 제한`,
    body: [
  ...
  {
    title: `${SUBSCRIPTION_BILLING_ENABLED ? "제15조" : "제14조"} 약관 변경`,
    body: [
