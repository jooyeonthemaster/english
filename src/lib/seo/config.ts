...
export const SITE = {
  ...
  defaultDescription:
    "영어 지문 분석, 내신·수능 24유형 문제 생성, Word·한글·PDF 시험지 자동 조판, 학생별 시험 리포트까지 끝내는 영어학원 AI 올인원 SMOAT(스모트)입니다.",
...
export const NOINDEX_PATH_PREFIXES = [
  ...
  "/r/", // 학생 시험 리포트 공유 링크(개인 토큰 URL) — 접근은 토큰 검증, 색인은 차단.
  //       트레일링 슬래시 필수: "/r" 이면 robots 프리픽스 매칭이 /refund-policy 등
  //       /r 로 시작하는 공개 페이지 크롤까지 차단한다(적대검수 실증).
] as const;
