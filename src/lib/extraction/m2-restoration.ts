...
export function buildGroundedRestorationPrompts(
  ...
  return {
    ...
    userPrompt: [
      ...
      "- #1 PROBLEM-COUPLED → PATCH from source. Triggers: diff at `(a)~(e)` underline AND question is GRAMMAR_* / VOCAB_CHOICE / CONTEXT_MEANING / REFERENCE; or diff inside `(A)~(D)` ordering chunk; or diff at a `________` / `(A)/(B)/(C)` blank.",
      ...
      "- Emit a `changes` entry ONLY for source patches (#1 #3): `before`=AI span, `after`=source span, `evidenceType` per class (GRAMMAR/VOCAB/BLANK/INSERTION/ORDERING/SUMMARY for #1, OTHER for #3). Do NOT emit changes for AI-only marker removal or evidence-driven blank fills handled in Investigation B.",
