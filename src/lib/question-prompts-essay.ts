...
- scrambledWords: 단어/구 목록 (배열). **반드시 정답 순서와 완전히 다르게 무작위로 뒤섞으세요.** 절대로 정답 순서대로 나열하지 마세요. 예를 들어 정답이 "A B C D E"라면 scrambledWords는 ["D", "B", "E", "A", "C"] 처럼 섞어야 합니다. 정답을 의미 단위로 **4~7개 청크로 고르게** 쪼개고, 한 청크가 정답의 절반 이상을 담지 않게 하세요(청크가 너무 크면 배열이 무의미).
...
- modelAnswer: 올바르게 배열된 완성 문장. 지문의 한 문장을 그대로 베낀 verbatim 이 아니라, 출제 의도에 맞는 문장이어야 합니다(지문 문장 통째 복사 금지 — 지문이 함께 보이면 베껴쓰기가 됨).
- correctAnswer: modelAnswer와 동일
- 난이도 차별화: **BASIC** = 2~4단어 짧은 구를 청크로 묶어 제시(쉬운 재배열), 미끼 없음. **INTERMEDIATE** = 단어 단위로 분해, 미끼 0~1개. **KILLER** = 단어 단위 + 강한 셔플 + 미끼 포함 + 긴 문장(절 구조).
