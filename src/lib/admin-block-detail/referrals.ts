...
import { REFERRAL_STATUS, statusOf } from "@/lib/admin-labels";
...
// 레지스트리에 없는 원본 status(스키마 변경·수기 수정 등)를 영문 그대로 흘리지 않는다.
// 무엇이었는지는 알아야 하므로 원상태를 괄호로 남긴다.
const statusLabel = (s: string) => statusOf(REFERRAL_STATUS, s, { label: `미분류(${s})`, tone: "gray" }).label;
...
> = {
  ...
  // 키 이름은 서버 액션 allowlist(STAT_KEYS)와 맞추려고 granted 를 유지한다 — 의미는 「지급 완료」.
  granted: {
    title: "지급 완료",
    subtitle:
      "보상이 실제로 지급된 추천 = 자동 지급(GRANTED) + 보류 심사 승인 지급(APPROVED). 최근 생성 순",
...
export async function referralStatDetail(key: ReferralStatKey): Promise<AdminDetail> {
  ...
  const columns: AdminDetailColumn[] = [
    ...
    ...(key === "total" || key === "granted" ? [{ key: "status", label: "상태" }] : []),
  ...
  // 「지급 완료」는 두 경로(자동/승인)의 합이므로 요약에 구성비를 덧붙인다 — 카드 보조 문구와 같은 값.
  if (key === "granted") {
    const split = await prisma.referral.groupBy({ by: ["status"], where, _count: { _all: true } });
    const byStatus = new Map(split.map((g) => [g.status, g._count._all]));
    detail.summary!.splice(1, 0, {
      label: "자동 / 승인 지급",
      value: `${num(byStatus.get("GRANTED") ?? 0)}건 / ${num(byStatus.get("APPROVED") ?? 0)}건`,
    });
  }
