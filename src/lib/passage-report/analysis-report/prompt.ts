...
- ❗ **반의어로 인정하는 것은 아래 셋이다. 사전적 반대말만 고집하지 마라.**
  ...
  · ③**이 지문이 직접 맞세운 대조축의 반대편 표현** — 지문이 A와 B를 명시적으로 대조하고 있을 때의 A↔B. 단, **지문 안에서 실제로 대조되고 있을 때만** 쓴다(글에 없는 대조를 상상해서 만들면 안 된다).
- ❗ **그래도 반대말이 성립하지 않는 표현만 "—" 로 둔다.** 중립 명사구·현상 명칭·전문 용어("media effect", "buying behavior" 류)가 여기 해당한다. **antonyms 키를 통째로 빼거나 빈 문자열로 두지 마라 — 그럴 때는 반드시 "—" 라는 글자를 넣어라.** (실제 사고: "media effect"의 반의어로 "cause"를 지어냈는데 그 "cause"가 같은 문제에 있던 "trigger"의 동의어라, 학생이 어떻게 답해도 오답이 되는 문항이 만들어졌다. 근거 없는 반의어보다는 "—"가 낫다.)
- ❗ **표제어를 고르는 단계에서부터 반의어를 함께 고려하라.** 학습 가치가 비슷한 후보가 둘이면 **반의어·대조어가 분명한 쪽을 먼저** 표제어로 넣어라(그쪽이 단어시험·연결 문제로 더 많이 쓰인다). 다만 이 이유로 **지문에 없는 단어를 표제어로 만드는 것은 절대 금지**다.
