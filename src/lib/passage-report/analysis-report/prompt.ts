...
{ "kind":"passage",
  "sentences":[ { "n":1, "en":"원문 문장(원문 그대로, 수정 금지)", "ko":"자연스러운 한국어 해석",
                  "chunks":[ {"text":"원문 그대로의 연속 구절","role":"짧은 한국어 구문 라벨","emphasis":"core 또는 생략"}, ... ] }, ... ],
...
- ❗ **chunks(직독직해 끊어읽기) — 권장**: 각 문장을 의미 단위(절/구)로 2~6조각 나눈다. 학생 노트처럼 각 구절 아래에 구문 라벨·필기가 정확히 붙도록 하는 핵심 데이터다.
  - 각 chunk = { "text": **원문 그대로의 연속 구절**, "role": 아주 짧은 한국어 구문 라벨("주절","이유 부사절","수단 전치사구","목적어절","관계절" 등), "emphasis": 글의 핵심 청크면 "core" }.
...
{ "kind":"grammar", "note":"※ ⚠ 는 시험에서 학생이 자주 틀리는 함정",
  "rows":[ { "sentenceNo":문장번호, "excerpt":"해당 자리가 든 실제 원문 구절", "pointCode":"a~m 중 하나", "point":"(코드) 분류 — 표현", "explanation":"정의→이유→비교→적용 4단계 쉬운 해설", "trap":"⚠ 함정/오답 형태",
...
- Q5 요약문 완성: 반드시 (A), (B) 두 칸 구조를 사용하고, 두 칸이 같은 논리축을 공유하도록 만드세요. 선택지는 (A)/(B) 쌍으로 5개를 제공하고, 하나만 맞도록 반의어·방향성·범위 차이를 섞으세요.
