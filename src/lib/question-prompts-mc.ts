...
## ⚠️ 해설 작성 금지 사항 (학생 관점)
- **출제 과정/생성 지침 노출 금지**: "지시문/가이드라인/출제 의도", "1순위 포인트를 활용", "함정으로 X를 Y로 변형하였습니다" 같은 *문제를 어떻게 만들었는지*는 절대 쓰지 마세요. 해설은 **그 형태가 왜 어법상 틀린지/옳은지만** 학생 관점에서 설명합니다. (❌ "...를 활용하여 interact를 to interact로 잘못 변형하였습니다" → ⭕ "이 명사절에는 정동사가 필요하므로 준동사 to interact는 올 수 없고 정동사 interact가 옳다")
...
- **keyPoints는 이 문항에 실제 등장한 표현만**: 다른 변형에서 가져온 무관한 포인트(이 문항에 없는 단어·보기) 잔재 금지.
