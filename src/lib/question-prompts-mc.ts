...
반드시 학생에게 보이는 틀린 표현(X)을 먼저 인용하고, 그 다음 교정형(Y)을 제시하세요. keyPoints는 이 문항이 실제로 검사한 문법 항목 3개를 학교문법 용어로.
오답(wrongOptionExplanations) 각 1문장: "이 자리는 [무엇]을 묻는다 — [구조 근거]이므로 '표현'이 어법상 옳다." 형식으로, 왜 매력적인 함정인지가 드러나게.
- **함정 서술의 사실 검증**: "바로 앞/뒤에 위치한 X에 이끌려" 같은 위치 주장은 지문의 실제 어순과 대조한 뒤에만 쓰세요 — 수 견인(attraction)을 언급하려면 그 명사가 실제로 **주어와 동사 사이**에 있어야 합니다. 지문에 없는 구조("동격/수식 구조를 걷어내면" 등)를 템플릿처럼 서술하면 허위 해설로 실패입니다. 함정 서술의 방향도 문항 형식과 일치시키세요: '틀린 것 찾기'에서 정답의 함정은 "옳다고 착각해 지나치기 쉽다"이지 "X를 골라 실수하기 쉽다"(정답 선택=실수)가 아닙니다.
