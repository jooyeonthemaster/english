...
핵심은 "표면의 말"과 "진짜 의미" 사이의 거리입니다. 그 거리가 거의 없으면 함축 의미 문제가 아니라 주제/요지 문제입니다.
...
## 선지 설계
1. options는 모두 한국어로 작성합니다.
2. 정답은 밑줄 표현의 직역이 아니라, 지문 근거를 종합한 함축 의미여야 합니다.
...
6. 절대어로 쉽게 제거되는 오답은 금지합니다. "완전히", "전적으로", "항상", "언제나", "오직", "무조건", "반드시", "예외 없이", "해야만", "만을/만이/만으로", "배제"와 "완전히 극복/해결/대체", "완벽하게 이해/설명/해결" 같은 표현은 원문이 그 정도를 명시하지 않는 한 쓰지 마세요.
7. KILLER 오답은 극단어 없이도 그럴듯해야 합니다. 예를 들어 "AI가 인간 노동을 완전히 대체한다", "감정을 배제해야 한다"처럼 바로 지워지는 선지 대신, "AI가 작업의 가치 기준 자체를 기계 수행 능력으로 옮긴다"처럼 지문 개념을 미묘하게 비트세요.
...
- options: label "1"~"5", text는 한국어 함축 의미 선택지
- wrongOptionExplanations: 정답이 아닌 모든 선지에 대해, 왜 그럴듯하지만 지문 근거상 틀리는지 한국어로 설명
- ⚠️ passageWithUnderline 필드는 생성하지 마세요 (서버에서 자동 생성)
- direction 예시: "밑줄 친 부분이 다음 글에서 의미하는 바로 가장 적절한 것은?"`,
