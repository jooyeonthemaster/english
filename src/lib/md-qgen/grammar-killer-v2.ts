...
2. 정답 = 후보 중 판정 단서가 가장 멀고, 오형이 로컬로 가장 자연스러운 자리. 지문에서 가장 구조가 복잡한 문장(삽입·관계절·병렬이 겹친 곳)에 두는 것이 원칙이다. 후보를 최소 2개 놓고 비교한 뒤 골라라. 수일치가 최선이더라도 주어의 핵과 동사 사이에 실제로 다른 명사·절이 끼어 있을 때만 허용한다.
   **하한 두 개를 반드시 통과시켜라(실기출 판단형 정답의 실측 하위 25% 선이다).** ① 정답 판정에 필요한 단서가 밑줄에서 **5단어 이상** 떨어져 있을 것 — 바로 옆 토큰이 답을 확정하면 킬러가 아니다. ② 밑줄과 그 단서 사이에 **관계절·삽입구·전치사구가 최소 1개** 끼어 있을 것. 두 조건을 못 채우는 자리밖에 없으면, 지문에서 가장 깊은 자리를 고르되 왜 그것이 최선인지 설계메모에 적어라.
