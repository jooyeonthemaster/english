...
// 26-08-21 실기출 98문항 정답 자리 전수 분류에서 도출. 최상위 변별을 하는 정답
// (판단형)은 **100%** 가 "오형만 떼어 읽으면 자연스러운" 자리였고, 보면 아는 정답
// (인지형)은 6.3% 만 그랬다. 즉 로컬 자연스러움이 킬러 정답의 사실상 필요조건이다.
// 판정 단서까지의 거리도 판단형 중앙 7단어 / 인지형 중앙 1단어로 갈렸다.
const RECOGNITION_BAN = `## 정답으로 쓰면 안 되는 자리 — "보면 아는" 오형 (실기출 분석)
아래는 오형을 심는 순간 **표면이 이미 부서져** 학생이 구조를 추적할 필요가 없어지는 자리다.
정답으로 쓰지 마라(미끼로도 약하다).
- 타동사 뒤에 목적어 없이 to부정사가 붙는 형태 — "have found to suffer", "is said to" 에서 be 를 지우는 류. 읽는 순간 목적어 결손이 보인다.
- 과거분사 뒤에 전치사 없이 「한정사+명사구」가 바로 이어지는 형태 — "approached a local art form" 류. 수동인데 목적어를 데리고 있어 표면이 파탄난다.
- 선행사 명사 바로 뒤의 what — "styles what were …" 류. 선행사가 눈앞에 있어 즉시 갈린다.
- 콤마 뒤에서 원형·현재형 정동사가 분사 자리를 차지하는 형태 — 앞 정동사가 3단어 안에 보이면 즉답이다.
- 접속사↔전치사 교체(because↔because of, despite↔although 류)를 **지문 전반부나 첫 밑줄 자리에** 두는 것 — 판정 단서(뒤따르는 명사구/절)가 밑줄 바로 옆이라 거리 하한을 정의상 통과하지 못하고, 첫 밑줄이면 학생이 가장 먼저 만나 즉시 종결해 나머지 네 자리를 검토하지 않는다(실측: 이 자리 하나로 미끼 검토 전멸). 기출에서 이 계열 정답은 지문 중후반 자리뿐이고, 기출 정답 라벨 분포는 ① 6.4%로 최희소다 — 정답 밑줄을 첫 자리에 두지 마라.
- 철자 오류·비단어·조동사 바로 뒤 비원형.

**출제 직전 자가검증(반드시 수행):** 정답 오형을 포함한 **5~6단어만 잘라내어** 읽어라.
그 조각만으로 어색하면 그 자리는 인지형이다 — 정답 후보에서 버리고 다시 골라라.
어색하지 않은데 문장 전체를 읽어야 비로소 틀린 것이 드러나야 한다.`;
...
- **예외 — 대용(代用) 계열(대동사 do/be·대명사·생략·비교)**: 판단 근거가 정의상 다른 절에 있으므로, 이때만 ① 대용어가 가리키는 대상과 ② 대신하는 원래 술어/명사를 **반드시 짚어라**. 그 근거 절의 해당 구간을 인용에 포함시켜, 지어낸 구조가 아니라 지문에 실재함을 보여라. (예: "others는 과거의 다른 합의들을 가리키며, 대신하는 술어는 앞 절의 be displaced(수동)이다 — do는 능동 술어만 대신하므로 have been으로 고친다.")

## 문법 용어 명명 규칙 (해설·오답 분석 공통 — 오명명 재발 실측 2회)
형태 명명은 실제 분류대로만 쓴다. 특히:
- 복수 주어에 수일치한 현재형 정동사(People cover 류)를 "동사원형"이라 부르지 마라 — 원형과 형태가 같아도 그것은 **현재형 동사**다. "동사원형"은 조동사·to·사역/지각동사 뒤 원형에만 쓴다.
- 동명사 주어(walking 류)를 "단수 명사구"라 부르지 마라 — **동명사(구) 주어**라 쓰고 단수 취급임을 밝힌다.
- 명명이 확실하지 않으면 품사명을 생략하고 역할로만 서술하라("주어에 수일치한 형태") — 틀린 명명은 그 자체가 학습 오류를 만든다.`;
...
2. 정답 = 후보 중 판정 단서가 가장 멀고, 오형이 로컬로 가장 자연스러운 자리. 지문에서 가장 구조가 복잡한 문장(삽입·관계절·병렬이 겹친 곳)에 두는 것이 원칙이다. 후보를 최소 2개 놓고 비교한 뒤 골라라. 수일치가 최선이더라도 주어의 핵과 동사 사이에 실제로 다른 명사·절이 끼어 있을 때만 허용한다.
   **하한 두 개를 반드시 통과시켜라(실기출 판단형 정답의 실측 하위 25% 선이다).** ① 정답 판정에 필요한 단서가 밑줄에서 **5단어 이상** 떨어져 있을 것 — 바로 옆 토큰이 답을 확정하면 킬러가 아니다. ② 밑줄과 그 단서 사이에 **관계절·삽입구·전치사구가 최소 1개** 끼어 있을 것. 두 조건을 못 채우는 자리밖에 없으면, 지문에서 가장 깊은 자리를 고르되 왜 그것이 최선인지 설계메모에 적어라. 그리고 후보 비교 때 **그 정답 선택이 남기는 미끼 벤치**도 함께 보라 — 정답 후보가 지문의 다른 강한 자리와 주어·구조를 공유해 그 자리를 미끼로 못 쓰게 만들면(그 미끼를 판정하는 순간 정답이 같이 풀림), 한 급 낮은 정답 + 살아있는 미끼 조합이 문항 전체로는 더 낫다(평가원이 같은 지문에서 이 이유로 정답 축을 옮긴 실례 있음).
3. 미끼 4개 = 각각 "학생이 구체적으로 무엇으로 잘못 고치고 싶어지는 자리". 그 잘못된 고침을 스스로 한 단어로 말할 수 없으면 그 자리는 미끼가 아니다(by -ing·관사 옆·주어 바로 옆 동사·지시 대상이 붙은 대명사는 실격). 정답과 같은 포인트코드는 피하되, **미끼끼리는 같은 코드를 최대 2개까지 허용한다** — 지문에 좋은 자리가 세 종류뿐이면 억지로 다섯 종류를 채우지 마라. 넷을 서로 다르게 만들려다 남는 칸을 약한 자리로 메우는 것이 실측된 최대 실패 원인이다.
   **미끼 자리 선호(실기출 미끼 624개 실측 분포)**: 관계사·명사절 18% > 수일치·대동사 14% > 분사 11% > to부정사/동명사 10% > 대명사 8% ≈ 형용사vs부사 8% > 목적격보어 7%. **관계사 자리를 1순위로 찾아라** — 기출 최다인데 가장 자주 빠뜨리는 자리다. 반대로 **형용사vs부사는 넷 중 최대 1개**까지만 쓴다.
   **「관사 + 밑줄 + 명사」 자리는 쓰지 마라** — 명사 앞 형용사는 대안 형태가 존재하지 않아 판단이 발생하지 않는다. 실기출 미끼 624개 중 5개(0.8%)뿐이고 정답으로 쓰인 적은 0건이다.
   **기준을 하나 더 통과시켜라 — 고침을 말할 수 있는 것으로는 부족하고, 그 고침이 그 자리에서 실제로 성립해 보여야 한다.** 규칙 하나를 아는 순간 대안이 사라지는 자리(사역·지각동사 뒤 원형, 계사 뒤 형용사, what is left 류 관용 고정표현, when/if + 과거분사 축약)는 학생이 검토조차 하지 않는다. 이런 **공식 자리는 넷 중 최대 1개**까지만 쓴다.
   그리고 **미끼 넷 중 최소 하나는 정답 자리와 같은 깊이로 만들어라** — 지배 요소(주어 핵·선행사·병렬 시작점)와 밑줄 사이에 관계절·삽입구·전치사구가 끼어 있고, 밑줄에 인접한 명사가 진짜 지배어와 수·격이 어긋나 보이는 자리. 실기출에서 미끼가 살아나는 것은 이 조건 하나 때문이고, 넷 다 공식 자리로 채우면 실질 선택지가 1개로 붕괴한다(실측 확인).
...
5. 검산: 마커를 원형으로 되돌리면 원문과 한 글자도 다르지 않은가. 정답 외 4곳이 표준 문법으로 이론의 여지 없이 옳은가(선행사가 둘로 읽히거나 원문 자체가 논쟁적인 자리는 정답으로도 미끼로도 쓰지 않는다). 해설·오답 해설이 서술하는 구조가 실제 지문에 그대로 있는가. **정답 오형을 그 자리에 둔 채 다른 통사 해석으로 다시 읽어라** — 축약관계절("accommodation booked"), 명사+명사 복합어("cook traditions"), 분사구문("your partner making") 어느 해석으로라도 성립하면 정답이 없는 문항이다. 폐기하고 다른 자리·축으로 옮겨라(비킬러 실측: 자리를 강제당한 산출물의 무효 정답 3건이 전부 이 유형이었다). 2단어 이상 밑줄과 동일한 문자열이 지문 다른 곳에 밑줄 없이 남아 있지 않은가 — 있으면 학생이 무표시 동일형과 대조만으로 푸니 그 자리는 교체한다(실사고: the "right to be forgotten" 밑줄과 같은 어구가 뒤 문장에 두 번 더 있었다).
