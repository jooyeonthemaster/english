...
- 정답은 지문의 표면 어휘를 재사용하지 않는 **추상 재진술**이다(핵심 명사·동사 재사용 0개 목표). 지문 문장·인용문을 그대로 옮겨 번역한 선지는 KILLER 에서 실격이다 — 출력 전에 정답 진술을 지문의 각 문장과 대조해, 어느 한 문장의 직역이면 두 마디(범위 조건 + 기능 관계)를 결합한 진술로 다시 써라(26-08-19 상한 재채점 실측: 주제문 직역 정답은 첫 문장 스캔으로 즉답된다).
- 상위권이 마지막까지 저울질할 오답이 최소 2개 있어야 한다. 그중 하나는 반드시 방향반대(핵심어는 그대로, 결론만 뒤집힌) 선지, 다른 하나는 **정답과 같은 긍정 방향이되 수단-목적이 전도되거나 범위 조건(예: '짧은/제한된'이라는 한정)을 위반한** 근접 선지다 — 정답만 유일한 극성이면 지문을 안 읽고도 풀린다.
