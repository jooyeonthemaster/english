...
function comboSelfCheck(mode: MdExplanationMode): string {
  return [
    ...
    "- 세 네모의 **틀린 표현**이 네모 밖 지문 어디에도 단어로 등장하지 않는지 확인하라 — 지문이 그 단어를 이미 합법적으로 쓰고 있으면(예: 부사 자리의 틀린 후보로 만든 형용사가 뒤 문장에 실존) 정답 시비로 반려된다. 등장하면 그 네모는 **다른 문장의 다른 포인트로 재선정**하라 — 지문은 수정할 수 없으므로 같은 후보쌍으로는 해소되지 않는다.",
