...
function closingBlock(shape: TswMdShape): string {
  ...
  const paraphraseRule = shape.sourceSentenceParaphrase
    ...
    : "";
  // 26-08-18 O225 해설 다이어트 — 해설 "딱 2문장" → 1~2문장(정답 도출만).
...
- 해설은 한국어만 쓴다(영단어를 한국어 문장의 어휘로 섞지 마라 — 지문 표현 인용만 허용). 합니다체 1~2문장 — 짧을수록 좋다.`;
}
...
- 보기 칩만으로 각 빈칸 정답을 실제로 조립할 수 있는지 하나씩 대조하라(전치사·관사까지). 같은 단어가 두 번 필요하면 칩도 두 개여야 한다.${
  ...
  // 26-08-18 O225 해설 다이어트 — 해설 검산에서 "학생이 어디서 흔들리는지" 심리 서사 지시 제거.
...
${modeCheck}
- 해설(1~2문장)이 주제를 어느 논리 흐름에서 도출했는지(정답 도출)만 말하는지 확인하라 — "학생이 어디서 흔들리는지" 같은 유혹·심리 서사는 쓰지 마라. 짧을수록 좋다.`;
}
...
function outputFormat(shape: TswMdShape, mode: MdExplanationMode): string {
  ...
  lines.push(
    mode === "answer-only"
      ...
      : "해설: <1~2문장 — 이 주제를 지문의 어느 논리 흐름에서 도출했는지(정답 도출만). \"학생이 어디서 흔들리는지\" 같은 유혹·심리 서사 금지. 합니다체>",
  );
  return lines.join("\n");
}
