...
- 표면 어휘 재사용 0개를 목표로 하라 — 지문의 핵심 명사·동사를 그대로 쓰지 말고 한 단계 추상화하거나, 지문이 만들어 낸 역설·비유를 제목의 형식으로 되살려라.
- 정답의 근거는 서로 다른 문장 두 개 이상에 흩어져 있어야 한다. 한 문장이 정답을 거의 그대로 풀어 놓은 지문이면 그 자리를 피해 논지 축을 다시 잡아라.
- 오답 중 최소 두 개는 상위권도 끝까지 정답과 저울질하게 만들어라 — 소재는 맞고 판단만 어긋난 선지가 그 역할을 한다.`,
};
