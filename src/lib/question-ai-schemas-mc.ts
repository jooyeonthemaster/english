...
export const aiGrammarErrorSchema = z.object({
  ...
  explanation: z
    .string()
    .describe(
      "정답 해설 (한국어, 200~450자, 합쇼체 통일, 4단 구조): ① 정답 문장의 골격 해부(진짜 주어/선행사/의미상 주어/병렬 짝을 이름 붙여 제시) ② 밑줄 표현이 그 자리에서 비문인 통사적 이유 단정 ③ '따라서 Y로 고쳐야 합니다' 교정 ④ (선택) 학생이 헷갈리는 함정 1문장. 반드시 학생에게 보이는 틀린 표현을 먼저 인용한 뒤 교정형을 제시. 출제 과정·변형 서사·내부 필드명 언급 금지.",
    ),
  keyPoints: z
    .array(z.string())
    .describe(
