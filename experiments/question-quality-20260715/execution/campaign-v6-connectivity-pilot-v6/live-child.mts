...
const { validateProtocolV6 } = protocolCoreExports;
const { assertExactLiveChildEnvironmentV6 } = liveEnvironmentExports;
...
const { runSealedConnectivityPilotV6 } = productionRunnerExports;
...
async function main(): Promise<void> {
  ...
  const protocol = validateProtocolV6(JSON.parse(readFileSync(path.join(packageRoot, "protocol-v6.json"), "utf8")) as unknown);
  ...
  const result = await runSealedConnectivityPilotV6({
    runId: args.runId,
