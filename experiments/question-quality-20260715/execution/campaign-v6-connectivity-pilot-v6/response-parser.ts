...
export interface BillingEvidenceV6 {
...
export interface ParsedResponseEvidenceV6 {
...
export function extractConnectivityBillingEvidenceV6(rawText: string): BillingEvidenceV6 {
  const defects: string[] = [];
  let duplicateBillingKeys = false;
  let strictObservationFailed = false;
  try {
    const duplicates = observeDuplicateJsonKeysV6(rawText);
  ...
  let costDisposition: BillingEvidenceV6["costDisposition"] = "ABSENT_OR_MALFORMED_UNKNOWN";
  ...
  if (duplicateBillingKeys) {
    ...
    explicitPositiveCostEvidenceHash = sha256V6(stableJsonV6({
      type: "duplicate-billing-keys",
      rawResponseSha256: sha256V6(rawText),
    }));
    ...
    if (positiveUpstreamCostEvidence.length > 0) {
      ...
      usage.cost <= MAX_PER_RESPONSE_ACTUAL_COST_USD_V6 &&
  ...
  if (usage && Object.hasOwn(usage, "completion_tokens_details")) {
    ...
    if (details && Object.keys(details).length === 1 && Object.hasOwn(details, "reasoning_tokens") &&
        ...
        details.reasoning_tokens >= 0 && details.reasoning_tokens <= MAX_PER_RESPONSE_USAGE_TOKENS_V6) {
  ...
  if (usage && Object.hasOwn(usage, "prompt_tokens_details")) {
    ...
    if (details && Object.keys(details).length === 1 && Object.hasOwn(details, "cached_tokens") &&
        ...
        details.cached_tokens >= 0 && details.cached_tokens <= MAX_PER_RESPONSE_USAGE_TOKENS_V6) {
  ...
  const core = {
    billingParserVersion: "campaign-v6-connectivity-pilot-billing-parser-v6",
  ...
  return { ...core, billingEvidenceHash: sha256V6(stableJsonV6(core)) };
}
...
function jsonEqual(left: unknown, right: unknown): boolean {
  return stableJsonV6(left) === stableJsonV6(right);
}
...
export function parseConnectivityResponseV6(input: {
  rawText: string;
  requestedModel: string;
...
}): ParsedResponseEvidenceV6 {
  const responseDuplicates = observeDuplicateJsonKeysV6(input.rawText);
  ...
  if (observeDuplicateJsonKeysV6(content).length > 0) {
  ...
  const billing = extractConnectivityBillingEvidenceV6(input.rawText);
  ...
  const evidenceCore = {
    parserVersion: "campaign-v6-connectivity-pilot-response-parser-v6",
    providerRequestId,
    ...
    finishReason,
    questionHash: sha256V6(stableJsonV6(question)),
    billingEvidenceHash: billing.billingEvidenceHash,
    responseSchemaSha256: sha256V6(stableJsonV6(input.responseSchema)),
    semanticQuestionCount: 1,
    choiceCount: 1,
    exactMessageShape: ["content", "role"],
  };
  return { ...evidenceCore, parserEvidenceHash: sha256V6(stableJsonV6(evidenceCore)) };
}
