import { parseConnectivityResponseV6, type ParsedResponseEvidenceV6 } from "./response-parser";
import type { JsonObject } from "./protocol-core";

export interface DeterministicLocalResponseV6 {
...
export interface DeterministicScenarioResultV6 {
  ...
  readonly evidence: readonly ParsedResponseEvidenceV6[];
}
 ...
 */
export function runDeterministicLocalScenarioV6(
  responses: readonly (DeterministicLocalResponseV6 | "FAIL_BEFORE_RESPONSE" | "UNKNOWN_AFTER_SEND")[],
): DeterministicScenarioResultV6 {
  const evidence: ParsedResponseEvidenceV6[] = [];
  let started = 0;
  let settled = 0;
  for (let index = 0; index < responses.length && index < 2; index += 1) {
    ...
    try {
      evidence.push(parseConnectivityResponseV6(response));
      settled += 1;
    } catch {
      settled += 1;
      return { terminal: "FAILED_TERMINAL", started, settled, successes: evidence.length, candidateOpportunities: started, physicalFetches: started, completions: started, evidence };
