...
export const LIVE_ENTRYPOINTS_V6 = [
...
const DECLARED_OPERATOR_BOOTSTRAP_ARTIFACTS = [
  ...
  const visit = (node: ts.Node): void => {
    if ((ts.isImportDeclaration(node) || ts.isExportDeclaration(node)) && node.moduleSpecifier && ts.isStringLiteralLike(node.moduleSpecifier)) {
      ...
      if (node.expression.kind === ts.SyntaxKind.ImportKeyword) {
        ...
        if (name === "readImmutableRepoBytesV6" || name === "readImmutableRepoJsonV6") {
        ...
        } else if (name === "readPrivateAttestedJsonV6" || name === "readPrivateAttestedJsonEvidenceV6") {
          dynamicPrivateReads += 1;
        } else if (name === "readDirectRealEnvLocalCredentialTextV6" &&
            slash(path.relative(repoRoot, file)).endsWith("operator-wrapper.mts")) {
          secretReads += 1;
        } else if (name === "withExclusiveRepoJsonTransactionV6") {
...
export function computeLiveClosureV6(input: {
  declaredRuntimeArtifactBytes?: ReadonlyMap<string, Uint8Array>;
} = {}) {
  const queue = LIVE_ENTRYPOINTS_V6.map((entry) => canonicalRepoFile(path.join(repoRoot, entry)));
  ...
  const operatorBootstrapRows = DECLARED_OPERATOR_BOOTSTRAP_ARTIFACTS
    .map((entry) => rowFor(entry, "immutable_runtime_artifact"));
  const files = [
    ...sourceRows, ...immutableRows, ...mutableRows,
    ...declaredRuntimeArtifactRows, ...operatorBootstrapRows,
  ]
  ...
  const core = {
    schemaVersion: "question-quality-connectivity-pilot-live-closure-v6",
    algorithm: "TYPESCRIPT_AST_TRANSITIVE_IMPORTS_PLUS_DECLARED_RUNTIME_AND_BOOTSTRAP_DATA_V2",
    entrypoints: [...LIVE_ENTRYPOINTS_V6],
    files,
    exactFileSetAndBytesSha256: SHA256(stableJsonV6(files)),
    ...
    runtimeInterfaces: {
      ...
      dynamicPrivateAttestedJsonContract: "canonical direct real private file; fd-bound exact-size pre/open/post identity read <=306184192 bytes; duplicate-free fatal UTF-8 without BOM; three raw bounded HTTP bodies reproduce the normalized schema-v6 snapshot; capture file SHA and bundle SHA handed to live child from same operator memory; <=15 minutes old",
      ...
      privateOutputContract: "exclusive real v6/private/runs/run directory with no symlink/junction ancestors; response and append-only journal never public",
    ...
    completeness: {
      ...
      externalOperatorBootstrapIncludedWithExactBytes: true,
  ...
  return { ...core, closureSemanticSha256: SHA256(stableJsonV6(core)) };
}
