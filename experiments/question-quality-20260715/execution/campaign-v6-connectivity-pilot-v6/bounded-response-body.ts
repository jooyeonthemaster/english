export const MODEL_RESPONSE_BODY_MAX_BYTES_V6 = 2 * 1024 * 1024;
export const METADATA_RESPONSE_BODY_MAX_BYTES_V6 = 16 * 1024 * 1024;
export const RESPONSE_BODY_MAX_CHUNKS_V6 = 8_192;

export type BoundedBodyFailureCodeV6 =
  | "INVALID_LIMIT"
...
export class BoundedBodyReadErrorV6 extends Error {
  readonly code: BoundedBodyFailureCodeV6;
  readonly maximumBytes: number;
  readonly observedOrDeclaredBytes: number | null;

  constructor(input: {
    code: BoundedBodyFailureCodeV6;
    label: string;
    maximumBytes: number;
    observedOrDeclaredBytes?: number | null;
  }) {
    super(`${input.label}: ${input.code}`);
    this.name = "BoundedBodyReadErrorV6";
...
function declaredLength(headers: Headers, label: string, maximumBytes: number): number | null {
  ...
  if (!/^(?:0|[1-9]\d*)$/u.test(raw)) {
    throw new BoundedBodyReadErrorV6({
...
export async function readBoundedUtf8ResponseBodyV6(input: {
  response: Response;
  maximumBytes: number;
  label: string;
}): Promise<{
  ...
  try {
    while (true) {
      ...
      if (next.done) break;
      chunksRead += 1;
      if (chunksRead > RESPONSE_BODY_MAX_CHUNKS_V6) {
        try {
