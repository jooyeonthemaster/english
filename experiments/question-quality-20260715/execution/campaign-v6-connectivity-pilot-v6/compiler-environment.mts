...
export const COMPILER_CHILD_MARKER_V6 = "QUESTION_QUALITY_CONNECTIVITY_PILOT_V6_COMPILER_CHILD";

export const COMPILER_OS_ENV_NAMES_V6 = [
  "COMSPEC",
  "PATH",
  "PATHEXT",
  "SystemRoot",
...
export const COMPILER_EXPLICIT_ENV_V6 = {
  [COMPILER_CHILD_MARKER_V6]: "1",
...
export interface CompilerEnvironmentRowV6 {
  name: string;
...
let droppedBeforeCompilerImportV6: string[] = [];

function sortedNames(value: NodeJS.ProcessEnv): string[] {
  return Object.keys(value).sort((left, right) => left < right ? -1 : left > right ? 1 : 0);
}

export function minimalCompilerEnvironmentV6(parent: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const child = {} as NodeJS.ProcessEnv;
  for (const name of COMPILER_OS_ENV_NAMES_V6) {
    const value = parent[name];
    if (typeof value === "string" && value.length > 0) child[name] = value;
  }
  for (const [name, value] of Object.entries(COMPILER_EXPLICIT_ENV_V6)) child[name] = value;
  return child;
}

export function sanitizeCompilerEnvironmentBeforeImportV6(env: NodeJS.ProcessEnv = process.env): string[] {
  const allowed = new Set([
    ...COMPILER_OS_ENV_NAMES_V6,
    ...Object.keys(COMPILER_EXPLICIT_ENV_V6),
  ]);
  const dropped = Object.keys(env).filter((name) => !allowed.has(name)).sort();
  for (const name of dropped) delete env[name];
  droppedBeforeCompilerImportV6 = dropped;
  return [...dropped];
}

export function assertAndDescribeIsolatedCompilerEnvironmentV6(
  env: NodeJS.ProcessEnv = process.env,
): {
  rows: CompilerEnvironmentRowV6[];
  ...
  const expected = minimalCompilerEnvironmentV6(env);
  ...
  for (const [name, value] of Object.entries(COMPILER_EXPLICIT_ENV_V6)) {
    if (env[name] !== value) throw new Error(`isolated compiler explicit value differs for ${name}`);
  }
  if (env[COMPILER_CHILD_MARKER_V6] !== "1") throw new Error("isolated compiler child marker missing");
  const rows: CompilerEnvironmentRowV6[] = actualNames.map((name) => {
    const value = env[name];
    if (typeof value !== "string") throw new Error(`isolated compiler environment value missing for ${name}`);
    return {
      name,
      kind: Object.hasOwn(COMPILER_EXPLICIT_ENV_V6, name) ? "explicit_offline_qgen" : "minimal_os",
  ...
  return {
    ...
    realCredentialValuesRead: 0,
    ambientEnvironmentVariablesAccepted: 0,
    ambientEnvironmentVariableNamesDroppedBeforeCompilerImport: [...droppedBeforeCompilerImportV6],
    droppedNameSetSha256: sha256(stable(droppedBeforeCompilerImportV6)),
    droppedCredentialValuesRead: 0,
  };
}
