...
export const ENV_LOCAL_MAX_BYTES_V6 = 4 * 1024 * 1024;

export interface CredentialReadRaceHooksV6 {
  afterOpenBeforeRead?: () => void;
  afterReadBeforePostAttestation?: () => void;
}

interface IdentityV6 {
  dev: bigint;
  ino: bigint;
  size: bigint;
  mtimeNs: bigint;
...
function sameNode(left: IdentityV6, right: IdentityV6): boolean {
  return left.dev === right.dev && left.ino === right.ino;
}

function sameFileIdentity(left: IdentityV6, right: IdentityV6): boolean {
 ...
 */
export function readDirectRealEnvLocalCredentialTextV6(
  repoRoot: string,
  candidatePath: string,
  raceHooks: CredentialReadRaceHooksV6 = {},
): string {
  ...
  if (!before.isFile() || before.isSymbolicLink() || before.size < BigInt(1) ||
      before.size > BigInt(ENV_LOCAL_MAX_BYTES_V6)) {
