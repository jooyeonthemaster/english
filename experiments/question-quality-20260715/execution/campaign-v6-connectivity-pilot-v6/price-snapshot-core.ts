import {
  ...
  type EndpointPriceV6,
  type JsonObject,
  type PublicPriceSnapshotV6,
} from "./protocol-core";
...
export interface RawPublicHttpResponseV6 {
  readonly url: string;
...
export interface RawPublicPriceInputsV6 {
  readonly fetchedAt: string;
  readonly modelsPayload: unknown;
  readonly endpointPayloads: Readonly<Record<string, unknown>>;
  readonly rawHttpResponses: readonly RawPublicHttpResponseV6[];
}

export interface PrivatePriceEvidenceBundleV6 extends JsonObject {
  schemaVersion: "question-quality-openrouter-private-price-evidence-bundle-v6";
  snapshot: PublicPriceSnapshotV6;
  rawHttpResponses: RawPublicHttpResponseV6[];
  bundleSha256: string;
}
...
function rawResponseCommitments(input: RawPublicPriceInputsV6): PublicPriceSnapshotV6["sources"]["rawResponseCommitments"] {
  const expectedUrls = [
    "https://openrouter.ai/api/v1/models",
    ...modelIdsV6().map((modelId) => `https://openrouter.ai/api/v1/models/${modelId}/endpoints`),
  ].sort();
  ...
  if (stableJsonV6(ordered.map((row) => row.url)) !== stableJsonV6(expectedUrls)) {
  ...
  const commitments = ordered.map((row) => {
    ...
    if (bodyUtf8Bytes < 1 || bodyUtf8Bytes > METADATA_RESPONSE_BODY_MAX_BYTES_V6) {
      throw new Error(`${row.url} raw HTTP body exceeds metadata bound`);
    }
    if (observeDuplicateJsonKeysV6(row.bodyText).length > 0) {
    ...
    return {
      ...
      bodyUtf8Bytes,
      bodySha256: sha256V6(row.bodyText),
    };
  });
  const modelsUrl = expectedUrls[0]!;
  if (stableJsonV6(parsedByUrl.get(modelsUrl)) !== stableJsonV6(input.modelsPayload)) {
  ...
  for (const modelId of modelIdsV6()) {
    ...
    if (stableJsonV6(parsedByUrl.get(url)) !== stableJsonV6(input.endpointPayloads[modelId])) {
...
const KNOWN_EXTRA_UNIT_DIMENSIONS_V6 = new Set([
  "image",
  "web_search",
  "internal_reasoning",
  "input_cache_read",
...
const INAPPLICABLE_UNIT_DIMENSIONS_V6 = new Set([
  "image",
  "web_search",
]);

const BOUNDED_INPUT_TOKEN_DIMENSIONS_V6 = new Set([
  "input_cache_read",
  "input_cache_write",
]);

const BOUNDED_OUTPUT_TOKEN_DIMENSIONS_V6 = new Set([
  "internal_reasoning",
]);

const REQUEST_NON_USE_CONTRACT_V6 = {
...
function pricingObject(
  ...
  if (pricing.request === undefined && !inherited) {
    throw new Error(`${label}.request is absent; v6 does not assume an omitted fixed request fee is zero`);
  }
  ...
  for (const [dimension, raw] of Object.entries(pricing)) {
    ...
    if (!KNOWN_EXTRA_UNIT_DIMENSIONS_V6.has(dimension)) {
...
function normalizeOverrides(
  ...
  basePricing: NormalizedPricingV6,
  label: string,
  chargeDimensions: Set<string>,
): EndpointPriceV6["overrides"] {
...
function normalizeEndpoint(endpoint: JsonObject, label: string, chargeDimensions: Set<string>): EndpointPriceV6 {
...
export function buildPublicPriceSnapshotV6(input: RawPublicPriceInputsV6): PublicPriceSnapshotV6 {
  const commitments = rawResponseCommitments(input);
  const requestedIds = modelIdsV6();
  ...
  const core = {
    schemaVersion: "question-quality-openrouter-public-price-snapshot-v6" as const,
    ...
    routingContract: {
      ...
      requestNonUseAndResponseAttestationContract: REQUEST_NON_USE_CONTRACT_V6,
    },
    chargeDimensions: [...chargeDimensions].sort(),
    knownInapplicableUnitChargeDimensions: [...chargeDimensions]
      .filter((dimension) => INAPPLICABLE_UNIT_DIMENSIONS_V6.has(dimension))
      .sort(),
    knownBoundedInputTokenChargeDimensions: [...chargeDimensions]
      .filter((dimension) => BOUNDED_INPUT_TOKEN_DIMENSIONS_V6.has(dimension))
      .sort(),
    knownBoundedOutputTokenChargeDimensions: [...chargeDimensions]
      .filter((dimension) => BOUNDED_OUTPUT_TOKEN_DIMENSIONS_V6.has(dimension))
      .sort(),
    models,
  };
  const snapshot = { ...core, contentSha256: sha256V6(stableJsonV6(core)) };
  return validatePriceSnapshotV6(snapshot);
}

export function buildPrivatePriceEvidenceBundleV6(
  snapshot: PublicPriceSnapshotV6,
  rawHttpResponses: readonly RawPublicHttpResponseV6[],
): PrivatePriceEvidenceBundleV6 {
  const validatedSnapshot = validatePriceSnapshotV6(snapshot);
  ...
  for (const commitment of validatedSnapshot.sources.rawResponseCommitments) {
    ...
    if (!raw || raw.status !== commitment.status || raw.contentType !== commitment.contentType ||
        ...
        sha256V6(raw.bodyText) !== commitment.bodySha256) {
      throw new Error(`private raw price evidence differs for ${commitment.url}`);
    }
    if (observeDuplicateJsonKeysV6(raw.bodyText).length > 0) {
      throw new Error(`private raw price evidence has duplicate keys for ${commitment.url}`);
    }
  }
  const core = {
    schemaVersion: "question-quality-openrouter-private-price-evidence-bundle-v6" as const,
    snapshot: validatedSnapshot,
    rawHttpResponses: [...rawHttpResponses].sort((left, right) => left.url.localeCompare(right.url, "en")),
  };
  return { ...core, bundleSha256: sha256V6(stableJsonV6(core)) };
}

export function validatePrivatePriceEvidenceBundleV6(value: unknown): PrivatePriceEvidenceBundleV6 {
  ...
  if (bundle.schemaVersion !== "question-quality-openrouter-private-price-evidence-bundle-v6" ||
  ...
  if (bundle.bundleSha256 !== sha256V6(stableJsonV6(core))) {
  ...
  const rawHttpResponses = bundle.rawHttpResponses.map((candidate, index) => {
      ...
      return row as unknown as RawPublicHttpResponseV6;
    });
  const byUrl = new Map(rawHttpResponses.map((row) => [row.url, JSON.parse(row.bodyText) as unknown]));
  const rebuiltSnapshot = buildPublicPriceSnapshotV6({
    fetchedAt: snapshot.fetchedAt,
    modelsPayload: byUrl.get("https://openrouter.ai/api/v1/models"),
    endpointPayloads: Object.fromEntries(modelIdsV6().map((modelId) => [
      modelId,
      byUrl.get(`https://openrouter.ai/api/v1/models/${modelId}/endpoints`),
    ])),
    rawHttpResponses,
  });
  if (stableJsonV6(rebuiltSnapshot) !== stableJsonV6(snapshot)) {
    throw new Error("private raw price evidence does not exactly reproduce normalized snapshot");
  }
  const rebuiltBundle = buildPrivatePriceEvidenceBundleV6(snapshot, rawHttpResponses);
  if (stableJsonV6(rebuiltBundle) !== stableJsonV6(bundle)) {
    throw new Error("private price evidence bundle is not canonical");
  }
  return rebuiltBundle;
}

export function validatePinnedPrivatePriceEvidenceBundleV6(input: {
  value: unknown;
  ...
  const bundle = validatePrivatePriceEvidenceBundleV6(input.value);
...
export function priceEvidenceForModelV6(snapshot: PublicPriceSnapshotV6, modelId: string): {
