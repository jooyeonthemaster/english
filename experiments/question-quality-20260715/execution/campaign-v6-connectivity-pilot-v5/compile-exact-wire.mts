...
import { computeCompilerClosureV5 } from "./compiler-closure.mts";
...
export const FROZEN_V4_WIRE_ROWS_V5 = {
  STANDARD: {
...
export async function compileConnectivityPilotExactWireV5() {
  ...
  // Its rejected v2 live runner is never imported into the v5 execution closure.
  const compiled = await compileV2ProductionWire();
  const upstream = compiled.privateArtifact;
  const compilerClosure = computeCompilerClosureV5();
  ...
  for (const row of rows) {
    const frozen = FROZEN_V4_WIRE_ROWS_V5[row.plan as keyof typeof FROZEN_V4_WIRE_ROWS_V5];
    assert(frozen, `v5 inherited an unexpected plan: ${row.plan}`);
  ...
  const privateCore = {
    schemaVersion: "question-quality-v6-connectivity-pilot-exact-wire-private-v5",
  ...
  const privateArtifact = {
    ...privateCore,
    privateSemanticSha256: sha256(stableJsonV5(privateCore)),
  };
  const privateBytes = `${JSON.stringify(privateArtifact, null, 2)}\n`;
  const publicCore = {
    schemaVersion: "question-quality-v6-connectivity-pilot-exact-wire-public-v5",
    status: "OFFLINE_PRODUCTION_CORE_EXACT_WIRE_LIVE_BLOCKED",
  ...
  const publicArtifact = { ...publicCore, publicSemanticSha256: sha256(stableJsonV5(publicCore)) };
...
async function main(): Promise<void> {
  throw new Error("v5 compiler has no ambient CLI; use compiler-child.mts through compiler-client.mts");
}
