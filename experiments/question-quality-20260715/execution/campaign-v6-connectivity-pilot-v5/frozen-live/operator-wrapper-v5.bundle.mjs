...
var credential_file_boundary_exports = {};
__export(credential_file_boundary_exports, {
  ENV_LOCAL_MAX_BYTES_V5: () => ENV_LOCAL_MAX_BYTES_V5,
  readDirectRealEnvLocalCredentialTextV5: () => readDirectRealEnvLocalCredentialTextV5
});
import {
  closeSync,
  constants,
...
var ENV_LOCAL_MAX_BYTES_V5 = 4 * 1024 * 1024;
...
function sameFileIdentity(left, right) {
  return sameNode(left, right) && left.size === right.size && left.mtimeNs === right.mtimeNs && left.ctimeNs === right.ctimeNs;
}
function readDirectRealEnvLocalCredentialTextV5(repoRoot2, candidatePath, raceHooks = {}) {
  const resolvedRoot = path.resolve(repoRoot2);
  const expectedPath = path.join(resolvedRoot, ".env.local");
  if (comparable(path.resolve(candidatePath)) !== comparable(expectedPath) || comparable(path.dirname(path.resolve(candidatePath))) !== comparable(resolvedRoot)) {
  ...
  if (!before.isFile() || before.isSymbolicLink() || before.size < BigInt(1) || before.size > BigInt(ENV_LOCAL_MAX_BYTES_V5)) {
    throw new Error("credential source must be a bounded real regular file");
  }
  if (comparable(realpathSync.native(path.dirname(expectedPath))) !== comparable(realRoot) || comparable(realpathSync.native(expectedPath)) !== comparable(expectedPath)) {
  ...
  try {
    ...
    if (!opened.isFile() || !afterOpen.isFile() || afterOpen.isSymbolicLink() || !sameFileIdentity(before, opened) || !sameFileIdentity(opened, afterOpen) || comparable(realpathSync.native(expectedPath)) !== comparable(expectedPath)) {
    ...
    if (!postDescriptor.isFile() || !postPath.isFile() || postPath.isSymbolicLink() || !rootAfter.isDirectory() || rootAfter.isSymbolicLink() || !sameNode(rootBefore, rootAfter) || !sameFileIdentity(opened, postDescriptor) || !sameFileIdentity(postDescriptor, postPath) || comparable(realpathSync.native(resolvedRoot)) !== comparable(realRoot) || comparable(realpathSync.native(path.dirname(expectedPath))) !== comparable(realRoot) || comparable(realpathSync.native(expectedPath)) !== comparable(expectedPath)) {
...
function assertFrozenRuntimeEntrypointV5(input) {
  ...
  if (!artifactStat.isFile() || artifactStat.isSymbolicLink() || comparable2(realpathSync2.native(artifactAbsolute)) !== comparable2(artifactAbsolute)) {
  ...
  for (const row of artifact.bundles) {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || comparable2(realpathSync2.native(absolute)) !== comparable2(absolute)) {
  ...
  if (comparable2(realpathSync2.native(input.currentModulePath)) !== comparable2(path2.resolve(input.repoRoot, ownRow.path))) {
...
var live_io_exports = {};
__export(live_io_exports, {
  PRIVATE_PRICE_EVIDENCE_BUNDLE_MAX_BYTES_V5: () => PRIVATE_PRICE_EVIDENCE_BUNDLE_MAX_BYTES_V5,
  assertDirectRealPrivateInputPathV5: () => assertDirectRealPrivateInputPathV5,
  assertJsonTransactionTargetUnchangedV5: () => assertJsonTransactionTargetUnchangedV5,
  assertRealDirectoryV5: () => assertRealDirectoryV5,
  assertRealRegularFileV5: () => assertRealRegularFileV5,
  attestJsonTransactionTargetV5: () => attestJsonTransactionTargetV5,
  classifyCommittedTransactionCleanupV5: () => classifyCommittedTransactionCleanupV5,
  createExclusivePrivateRunDirectoryV5: () => createExclusivePrivateRunDirectoryV5,
  livePackageRootV5: () => livePackageRootV5,
  livePrivateRootV5: () => livePrivateRootV5,
  liveRepoRootV5: () => liveRepoRootV5,
  readImmutableRepoBytesV5: () => readImmutableRepoBytesV5,
  readImmutableRepoJsonV5: () => readImmutableRepoJsonV5,
  readPrivateAttestedJsonEvidenceV5: () => readPrivateAttestedJsonEvidenceV5,
  readPrivateAttestedJsonV5: () => readPrivateAttestedJsonV5,
  readStableMutableJsonV5: () => readStableMutableJsonV5,
  withExclusiveRepoJsonTransactionV5: () => withExclusiveRepoJsonTransactionV5
});
import {
...
var liveModuleDirectoryV5 = path3.dirname(fileURLToPath(import.meta.url));
var livePackageRootV5 = path3.basename(liveModuleDirectoryV5) === "frozen-live" ? path3.dirname(liveModuleDirectoryV5) : liveModuleDirectoryV5;
var liveRepoRootV5 = path3.resolve(livePackageRootV5, "../../../..");
var livePrivateRootV5 = path3.join(livePackageRootV5, "private");
...
function assertRealDirectoryV5(directoryPath, label) {
  ...
  if (comparable3(realpathSync3.native(absolute)) !== comparable3(absolute)) {
...
function assertRealRegularFileV5(filePath, label) {
  ...
  assertRealDirectoryV5(path3.dirname(absolute), `${label} parent`);
  return absolute;
}
function createExclusivePrivateRunDirectoryV5(runId) {
  ...
  if (comparable3(path3.dirname(runRoot)) !== comparable3(runsRoot)) {
...
function resolveRepoPath(relativePath) {
  ...
  const absolute = path3.resolve(liveRepoRootV5, relativePath);
  const relative = path3.relative(liveRepoRootV5, absolute);
...
function readImmutableRepoBytesV5(relativePath) {
  ...
  const size = lstatSync3(canonical).size;
...
function readImmutableRepoJsonV5(relativePath) {
  return JSON.parse(readImmutableRepoBytesV5(relativePath).toString("utf8"));
}
function assertDirectRealPrivateInputPathV5(filePath, rootPath = livePrivateRootV5) {
  ...
  const absolute = path3.resolve(filePath);
  const relative = path3.relative(privateRoot, absolute);
  if (!relative || relative.startsWith("..") || path3.isAbsolute(relative) || path3.extname(absolute) !== ".json" || comparable3(path3.dirname(absolute)) !== comparable3(privateRoot)) {
...
function readPrivateAttestedJsonV5(filePath) {
...
function readPrivateAttestedJsonEvidenceV5(filePath, raceHooks = {}) {
  ...
  const parent = path3.dirname(canonical);
  const before = lstatSync3(canonical, { bigint: true });
  const parentBefore = lstatSync3(parent, { bigint: true });
  ...
  const fd = openSync2(canonical, constants2.O_RDONLY | noFollow);
  const bytes = Buffer.alloc(Number(before.size));
  const extra = Buffer.alloc(1);
  try {
    ...
    let offset = 0;
    while (offset < bytes.byteLength) {
      const count = readSync2(fd, bytes, offset, bytes.byteLength - offset, null);
      if (count === 0) throw new Error("attested private price evidence truncated during bounded read");
      offset += count;
    }
    if (readSync2(fd, extra, 0, 1, null) !== 0) {
      throw new Error("attested private price evidence grew during bounded read");
    }
    raceHooks.afterReadBeforePostAttestation?.();
    const postDescriptor = fstatSync2(fd, { bigint: true });
    const postPath = lstatSync3(canonical, { bigint: true });
    const parentAfter = lstatSync3(parent, { bigint: true });
    if (!postDescriptor.isFile() || !postPath.isFile() || postPath.isSymbolicLink() || !sameFileIdentityV5(opened, postDescriptor) || !sameFileIdentityV5(postDescriptor, postPath) || !sameNodeIdentityV5(parentBefore, parentAfter) || comparable3(realpathSync3.native(parent)) !== comparable3(parent) || comparable3(realpathSync3.native(canonical)) !== comparable3(canonical)) {
...
function attestJsonTransactionTargetV5(targetPath) {
  ...
  const parentPath = assertRealDirectoryV5(path3.dirname(target), "repository transaction parent");
  const targetStat = lstatSync3(target, { bigint: true });
  const parentStat = lstatSync3(parentPath, { bigint: true });
...
function assertJsonTransactionTargetUnchangedV5(attestation) {
  ...
  if (comparable3(current.targetPath) !== comparable3(attestation.targetPath) || comparable3(current.parentPath) !== comparable3(attestation.parentPath) || !sameFileIdentityV5(current.targetIdentity, attestation.targetIdentity) || !sameNodeIdentityV5(current.parentIdentity, attestation.parentIdentity)) {
...
function readStableMutableJsonV5(attestation, hooks) {
  const noFollow = constants2.O_NOFOLLOW ?? 0;
  const fd = openSync2(attestation.targetPath, constants2.O_RDONLY | noFollow);
  ...
  try {
    ...
    const post = fstatSync2(fd, { bigint: true });
    const postIdentity = {
      dev: post.dev,
      ino: post.ino,
      size: post.size,
...
function withExclusiveRepoJsonTransactionV5(input) {
  ...
  const lockHandle = openSync2(lockPath, "wx", 384);
  let committed = false;
  let result;
...
var credentialBoundaryExports = void 0 ?? credential_file_boundary_exports;
var liveEnvironmentExports = void 0 ?? live_environment_exports;
var authorGateExports = void 0 ?? author_freeze_gate_exports;
var liveIoExports = void 0 ?? live_io_exports;
var priceSnapshotExports = void 0 ?? price_snapshot_core_exports;
var { metadataNetworkDispatchAuthorizedV5: metadataNetworkDispatchAuthorizedV52, validateProtocolV5: validateProtocolV52 } = protocolCoreExports;
...
var { readDirectRealEnvLocalCredentialTextV5: readDirectRealEnvLocalCredentialTextV52 } = credentialBoundaryExports;
var { buildExactLiveChildEnvironmentV5: buildExactLiveChildEnvironmentV52, buildExactMetadataEnvironmentV5: buildExactMetadataEnvironmentV52 } = liveEnvironmentExports;
var { assertAuthorFreezePermanentlyNoDispatchV5: assertAuthorFreezePermanentlyNoDispatchV52 } = authorGateExports;
var { readPrivateAttestedJsonEvidenceV5: readPrivateAttestedJsonEvidenceV52 } = liveIoExports;
var { validatePrivatePriceEvidenceBundleV5: validatePrivatePriceEvidenceBundleV52, validatePinnedPrivatePriceEvidenceBundleV5: validatePinnedPrivatePriceEvidenceBundleV52 } = priceSnapshotExports;
var moduleDirectory = path4.dirname(fileURLToPath2(import.meta.url));
...
var protocolPath = path4.join(packageRoot, "protocol-v5.json");
var childPath = path4.join(packageRoot, "frozen-live/live-child-v5.bundle.mjs");
var metadataCapturePath = path4.join(packageRoot, "frozen-live/capture-price-snapshot-v5.bundle.mjs");
var envLocalPath = path4.join(repoRoot, ".env.local");
var priceCaptureHandoffsV5 = /* @__PURE__ */ new Map();
function canonicalPricePath(value) {
  return path4.resolve(value);
}
function waitForChild(child, signalLabel) {
...
function assertFrozenOperator(protocol) {
  const reference = frozenRuntimeReferenceFromProtocolV52(protocol);
...
function decodeAssignmentValue(raw) {
  ...
  if (value.startsWith("'") || value.startsWith('"')) {
    ...
    if (quote === '"') return inner.replace(/\\([\\"nrt])/gu, (_match, token) => ({ "\\": "\\", '"': '"', n: "\n", r: "\r", t: "	" })[token]);
    return inner;
  }
  if (/\s/u.test(value) || /[#`$]/u.test(value)) throw new Error("unquoted OPENROUTER_API_KEY contains unsupported syntax");
  return value;
}
function readOnlyOpenRouterAssignment() {
  const text = readDirectRealEnvLocalCredentialTextV52(repoRoot, envLocalPath);
  let retained = null;
...
async function launchConnectivityPilotV5AfterFutureAuthorization(input) {
  assertAuthorFreezePermanentlyNoDispatchV52();
  const protocol = validateProtocolV52(JSON.parse(readFileSync3(protocolPath, "utf8")));
  const authorization = protocol.authorization;
  if (authorization.liveExecutionAuthorized !== true || authorization.hostileAuditPassed !== true || authorization.dispatchCommandPresent !== true) {
  ...
  const observed = readPrivateAttestedJsonEvidenceV52(canonicalSnapshotPath);
  validatePinnedPrivatePriceEvidenceBundleV52({
  ...
  const child = spawn(process.execPath, [
    ...
    cwd: repoRoot,
    env: buildExactLiveChildEnvironmentV52(process.env, key),
    stdio: ["ignore", "inherit", "inherit"],
    windowsHide: true
  });
  return await waitForChild(child, "live");
}
async function launchPriceMetadataCaptureV5AfterFutureAuthorization(input) {
  assertAuthorFreezePermanentlyNoDispatchV52();
  const protocol = validateProtocolV52(JSON.parse(readFileSync3(protocolPath, "utf8")));
  ...
  const child = spawn(process.execPath, [
    metadataCapturePath,
    `--output=${path4.resolve(input.outputPath)}`
  ], {
    cwd: repoRoot,
    env: buildExactMetadataEnvironmentV52(process.env),
    stdio: ["ignore", "inherit", "inherit"],
    windowsHide: true
  });
  const exitCode = await waitForChild(child, "metadata capture");
  if (exitCode === 0) {
    const observed = readPrivateAttestedJsonEvidenceV52(canonicalPricePath(input.outputPath));
    const bundle = validatePrivatePriceEvidenceBundleV52(observed.value);
...
async function blockedEntrypoint() {
  assertAuthorFreezePermanentlyNoDispatchV52();
}
...
export {
  launchConnectivityPilotV5AfterFutureAuthorization,
  launchPriceMetadataCaptureV5AfterFutureAuthorization
};
