...
import { assertAndDescribeIsolatedCompilerEnvironmentV5 } from "./compiler-environment.mts";
...
export interface CompilerClosureRowV5 {
  path: string;
...
export const DECLARED_COMPILER_DATA_INPUTS_V5 = [
...
function sourceRow(relative: string, kind: CompilerFileKind): CompilerClosureRowV5 {
...
function compilerFileSystemCallSites(sourceRows: readonly CompilerClosureRowV5[]) {
  ...
  return {
    contentReads,
    metadataReads,
    contentReadsSha256: hash(stableJsonV5(contentReads)),
    metadataReadsSha256: hash(stableJsonV5(metadataReads)),
  };
}
...
export function computeCompilerClosureV5() {
  const queue = COMPILER_ENTRYPOINTS_V5.map((entry) => canonicalRepoFile(path.join(repoRoot, entry)));
  const visited = new Set<string>();
  const external = new Set<string>();
  const unresolvedLocalSpecifiers: Array<{ from: string; specifier: string }> = [];
  const nonliteralDynamicLoads: string[] = [];
  const edges: ImportEdgeV5[] = [];
  ...
  while (queue.length > 0) {
    ...
    const imports: Array<{ specifier: string; kind: ImportEdgeV5["kind"] }> = [];
    const visit = (node: ts.Node): void => {
      ...
      if (
        ...
        const kind: ImportEdgeV5["kind"] = node.expression.kind === ts.SyntaxKind.ImportKeyword
  ...
  const dynamicSourceInputs = [...DYNAMIC_COMPILER_SOURCE_INPUTS_V5]
    .sort()
    .map((relative) => sourceRow(relative, "dynamic_compiler_source"));
  const declaredDataInputs = [...DECLARED_COMPILER_DATA_INPUTS_V5].sort().map((relative) => sourceRow(relative, "declared_compiler_data"));
  ...
  assert.deepEqual(
    ...
    [...DYNAMIC_COMPILER_SOURCE_INPUTS_V5].sort(),
  ...
  const environment = assertAndDescribeIsolatedCompilerEnvironmentV5();
  const git = exactGitInput();
  const core = {
    schemaVersion: "question-quality-connectivity-pilot-compiler-closure-v5",
    algorithm: "INDEPENDENT_AST_PLUS_DYNAMIC_SOURCE_PLUS_DATA_PLUS_EXTERNAL_INPUTS_V3",
    entrypoints: [...COMPILER_ENTRYPOINTS_V5],
    sourceFiles,
    dynamicSourceInputs,
    declaredDataInputs,
    files,
    exactSourceFileSetAndBytesSha256: hash(stableJsonV5(sourceFiles)),
    exactDynamicSourceSetAndBytesSha256: hash(stableJsonV5(dynamicSourceInputs)),
    exactDeclaredDataSetAndBytesSha256: hash(stableJsonV5(declaredDataInputs)),
    exactCompilerClosureSetAndBytesSha256: hash(stableJsonV5(files)),
    resolutionEvidence: {
      edges: orderedEdges,
      edgesSha256: hash(stableJsonV5(orderedEdges)),
    ...
    externalInputContract: {
      git,
      environment,
      exactExternalInputsSha256: hash(stableJsonV5({ git, environment })),
    ...
    fileSystemInputContract: {
      ...
      dynamicSourceInputs: [...DYNAMIC_COMPILER_SOURCE_INPUTS_V5],
      declaredImmutableDataInputs: [...DECLARED_COMPILER_DATA_INPUTS_V5],
  ...
  return { ...core, compilerClosureSemanticSha256: hash(stableJsonV5(core)) };
}
