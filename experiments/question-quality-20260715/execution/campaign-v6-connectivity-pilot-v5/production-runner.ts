...
import {
  ...
  validateProtocolV5,
  type AssignmentV5,
  type ConnectivityPilotProtocolV5,
  type JsonObject,
  type PilotPlanV5,
} from "./protocol-core";
import { priceEvidenceForModelV5, validatePinnedPrivatePriceEvidenceBundleV5 } from "./price-snapshot-core";
import {
  extractConnectivityBillingEvidenceV5,
  parseConnectivityResponseV5,
  type ParsedResponseEvidenceV5,
...
import {
  ...
  MAX_RUN_EFFECTIVE_COST_USD_V5,
  MAX_RUN_USAGE_TOKENS_V5,
  projectCandidateCapacityQuarantineV5,
  type AssignmentChargeV5,
  type RunLedgerSettlementV5,
} from "./terminal-reconciliation-core";
import { observeRawCandidateCardinalityV5 } from "./strict-json-observer";
import { assertAuthorFreezePermanentlyNoDispatchV5 } from "./author-freeze-gate";
import {
  assertFrozenRuntimeEntrypointV5,
  frozenRuntimeReferenceFromProtocolV5,
} from "./frozen-runtime-core";
import { LIVE_CHILD_MARKER_ENV_V5 } from "./live-environment";
import {
  assertRealDirectoryV5,
  createExclusivePrivateRunDirectoryV5,
  livePackageRootV5,
  readImmutableRepoBytesV5,
  readPrivateAttestedJsonEvidenceV5,
  withExclusiveRepoJsonTransactionV5,
} from "./live-io";

const directNetworkFetchV5 = globalThis.fetch.bind(globalThis);
const PROTOCOL_PATH = "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/protocol-v5.json";
const EXACT_WIRE_PATH = "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/private/exact-wire-v5.private.json";
const GLOBAL_LEDGER_PATH = "experiments/question-quality-20260715/budget-ledger.json";
const ENDPOINT = "https://openrouter.ai/api/v1/chat/completions";
const GLOBAL_BATCH_ID = "campaign-v6-connectivity-pilot-v5-two-call";

interface ExactWireRowV5 {
  ordinal: 1 | 2;
  plan: PilotPlanV5;
  modelId: string;
  endpoint: string;
...
interface ExactWireArtifactV5 extends JsonObject {
...
interface GlobalLedgerV5 extends JsonObject {
...
interface JournalEventV5 extends JsonObject {
  sequence: number;
  at: string;
  event: string;
  plan: PilotPlanV5 | null;
  details: JsonObject;
  previousHash: string;
  eventHash: string;
}

interface PrivateRunStateV5 extends JsonObject {
  ...
  charges: AssignmentChargeV5[];
  journalHeadHash: string;
}

export interface PublicConnectivityResultV5 {
...
function addMoney(left: number, right: number): number {
  ...
  if (!Number.isSafeInteger(totalScaled) || totalScaled < 0 ||
      totalScaled > MAX_RUN_EFFECTIVE_COST_USD_V5 * 1e12) {
...
function exactWire(): { protocol: ConnectivityPilotProtocolV5; artifact: ExactWireArtifactV5 } {
  ...
  const artifact = parseJsonBytes(artifactBytes, "private exact-wire artifact") as ExactWireArtifactV5;
  if (artifact.schemaVersion !== "question-quality-v6-connectivity-pilot-exact-wire-private-v5" ||
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (wire.ordinal !== assignment.ordinal || wire.plan !== assignment.plan || wire.modelId !== assignment.modelId ||
        wire.endpoint !== ENDPOINT || wire.bodySha256 !== sha256V5(wire.bodyText) ||
    ...
    if (body.model !== assignment.modelId || (body.n !== undefined && body.n !== 1) || body.stream === true ||
        stableJsonV5(body.provider) !== stableJsonV5({
    ...
    if (responseFormat.type !== "json_schema" || jsonSchema.strict !== true ||
        sha256V5(stableJsonV5(responseSchema)) !== wire.schemaSha256) {
...
function assertAuthorized(protocol: ConnectivityPilotProtocolV5): void {
  const authorization = protocol.authorization as unknown as Record<string, unknown>;
  if (process.env[LIVE_CHILD_MARKER_ENV_V5] !== "1" || authorization.liveExecutionAuthorized !== true ||
...
function validateFreshPriceSnapshot(
  protocol: ConnectivityPilotProtocolV5,
  snapshotPath: string,
...
function globalLedger(value: unknown): GlobalLedgerV5 {
  const ledger = asObject(value, "global research ledger") as GlobalLedgerV5;
  const exactKeys = [
...
function settleGlobal(input: RunLedgerSettlementV5): string[] {
  const outcome = withExclusiveRepoJsonTransactionV5({
    relativePath: GLOBAL_LEDGER_PATH,
    lockSuffix: "campaign-v6-connectivity-pilot-v5",
    mutate(current) {
      ...
      if (!Number.isSafeInteger(input.used) || input.used < 0 || input.used > 2 || input.modelCalls !== input.used ||
          ...
          !Number.isSafeInteger(input.inputTokens) || input.inputTokens < 0 || input.inputTokens > MAX_RUN_USAGE_TOKENS_V5 ||
          !Number.isSafeInteger(input.outputTokens) || input.outputTokens < 0 || input.outputTokens > MAX_RUN_USAGE_TOKENS_V5 ||
          !Number.isFinite(input.actualCostUsd) || input.actualCostUsd < 0 || input.actualCostUsd > MAX_RUN_EFFECTIVE_COST_USD_V5 ||
          !Number.isFinite(input.effectiveCostUsd) || input.effectiveCostUsd < 0 || input.effectiveCostUsd > MAX_RUN_EFFECTIVE_COST_USD_V5 ||
...
function quarantineGlobalCandidateCapacity(input: RunLedgerSettlementV5): string[] {
...
function createPrivateState(runId: string): {
  runRoot: string;
  state: PrivateRunStateV5;
  append: (event: string, plan: PilotPlanV5 | null, details: JsonObject) => void;
  ...
  const writeMarker = (name: string, value: JsonObject) => {
    ...
    const marker = openSync(path.join(runRoot, name), "wx", 0o600);
    try {
  ...
  let sequence = 0;
  const append = (event: string, plan: PilotPlanV5 | null, details: JsonObject) => {
    ...
    const row: JournalEventV5 = { ...core, eventHash: sha256V5(stableJsonV5(core)) };
  ...
  const persistFinal = () => {
    ...
    const stateHandle = openSync(statePath, "wx", 0o600);
    try {
      writeFileSync(stateHandle, `${JSON.stringify(state, null, 2)}\n`, "utf8");
      fsyncSync(stateHandle);
    } finally {
      closeSync(stateHandle);
    }
  };
  return { runRoot, state, append, writeMarker, persistFinal };
}

function publicResult(protocol: ConnectivityPilotProtocolV5, state: PrivateRunStateV5): PublicConnectivityResultV5 {
  const evidence = state.assignments.map((row) => row.parserEvidence).filter(Boolean) as ParsedResponseEvidenceV5[];
  ...
  const core = {
    schemaVersion: "question-quality-v6-connectivity-pilot-public-result-v5" as const,
...
async function executeOne(input: {
  ...
  let rawText = "";
  let parserEvidence: ParsedResponseEvidenceV5 | null = null;
...
export async function runSealedConnectivityPilotV5(input: {
  runId: string;
  priceSnapshotPath: string;
  priceSnapshotFileSha256: string;
  priceSnapshotBundleSha256: string;
}): Promise<PublicConnectivityResultV5> {
  ...
  assertFrozenRuntimeEntrypointV5({
    repoRoot: path.resolve(livePackageRootV5, "../../../.."),
