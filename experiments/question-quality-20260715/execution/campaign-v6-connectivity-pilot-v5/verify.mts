...
import { COMPILER_EXPLICIT_ENV_V5, COMPILER_OS_ENV_NAMES_V5 } from "./compiler-environment.mts";
...
const { stableJsonV5, validateProtocolV5 } = protocolCoreExports;
const frozenBuildExports =
  (frozenBuildModule as unknown as { default?: typeof frozenBuildModule }).default ?? frozenBuildModule;
...
const { buildFrozenRuntimeV5 } = frozenBuildExports;
const {
...
const liveDataInputs = [
  ...
  ["experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/frozen-live/capture-price-snapshot-v5.bundle.mjs", "immutable_runtime_artifact"],
  ["experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/frozen-live/live-child-v5.bundle.mjs", "immutable_runtime_artifact"],
  ["experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/frozen-live/operator-wrapper-v5.bundle.mjs", "immutable_runtime_artifact"],
  ["experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/frozen-runtime-v5.json", "immutable_runtime_artifact"],
  ["experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/private/exact-wire-v5.private.json", "immutable_runtime_data"],
  ["experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v5/protocol-v5.json", "immutable_runtime_data"],
] as const;
...
function assertExactRows(candidate: unknown, expected: unknown, label: string): void {
  assert.equal(stableJsonV5(candidate), stableJsonV5(expected), `${label} requires exact set/bytes/hash equality; subsets and minimum-count substitutes are forbidden`);
}
...
function verifyManifest(): number {
  ...
  for (const row of rows) {
    ...
    if (relativePath!.endsWith(".json")) {
      assert.notDeepEqual([...bytes.subarray(0, 3)], [0xef, 0xbb, 0xbf], `manifest JSON has UTF-8 BOM: ${relativePath}`);
      const text = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
      assert.doesNotThrow(() => JSON.parse(text), `manifest JSON is not exact fatal-UTF8 parseable: ${relativePath}`);
    }
  }
  return rows.length;
}
...
function verifyV4IndependentAuditPins(): void {
  const directory = "experiments/question-quality-20260715/reviews/campaign-v6-connectivity-pilot-v4-independent-audit-v1";
  const expectedRows: Record<string, string> = {
    [`${directory}/author-gates.json`]: "5b0346fb581b5cd12381484061fee8b4fc17e56a10882d7e29651ee8cbf613b0",
    [`${directory}/build.mts`]: "45d21b186aac9d39f928bbbf5e65c787cd602ad06968131c4dae7d424b5d3f2a",
    [`${directory}/evidence.json`]: "2aff0d69c81169f2f806dcd59f9053ce939927603d75cb7d7c2a60cab501f3b6",
    [`${directory}/independent-audit.mts`]: "1b6ce550cf2e109f106834695318460ca8a6eec2d5a80a9e7e706520317140d6",
    [`${directory}/README.md`]: "dcc562b826a79618006482af4bfff209832715af5d0f6a43ec1e0fad66b25e6e",
    [`${directory}/report.json`]: "8f135466efc5182843cf5a257c5bb361cafb2c91122798c29768839d0f31fae3",
    [`${directory}/run-author-gates.mts`]: "b5e5fb904a9f6f11fb7d25e402584099ffbf4ddbdc64ae5eb93133173328ae29",
    [`${directory}/tsconfig.json`]: "1b2c619dd51b41bbc1b05c5f8753a2fe5440bba07f58effd3a21a134f66bc48a",
    [`${directory}/verify.mts`]: "2211da72c2249c1302eddac8f138acf91c4c302a36fbb7810d24f20035c73dd0",
  };
  const manifestBytes = readFileSync(path.join(repoRoot, directory, "MANIFEST.sha256"));
  assert.equal(sha256(manifestBytes), "2b7fb398cad98647698c7dd4120f819c50645116c71e8ffe79ccb53530ba0791");
  ...
  assert.equal(sha256(readFileSync(path.join(repoRoot, "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v4/protocol-v4.json"))), "18d7eb7c60fdfc1d4b49fb868ef887d64e62db6d217abf8eaec0fbbeab8ffec9");
  const report = JSON.parse(readFileSync(path.join(repoRoot, directory, "report.json"), "utf8")) as Record<string, unknown>;
...
function manifestBoundContentDigest(): string {
  ...
  const exact = rows.map((row) => {
    const match = /^([a-f0-9]{64})  (.+)$/u.exec(row);
    assert(match);
    return { path: match[2], sha256: sha256(readFileSync(path.join(repoRoot, match[2]!))) };
  });
  return sha256(stableJsonV5(exact));
}

function assertNoNondeterministicGeneratedFields(relativePaths: readonly string[]): void {
  for (const relative of relativePaths) {
    ...
    const visit = (value: unknown): void => {
      ...
      for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
        assert.doesNotMatch(key, /duration|elapsed|wall.?clock|random.?temp|temporary.?directory/iu, `${relative}:${key}`);
        visit(child);
      }
    };
    visit(JSON.parse(raw) as unknown);
    assert.doesNotMatch(raw, /[A-Z]:\\[^"\r\n]*\\Temp\\qgen-connectivity-v5-compiler-|\/tmp\/qgen-connectivity-v5-compiler-/iu, relative);
  }
}
...
async function main(): Promise<void> {
  ...
  globalThis.fetch = (async () => { deniedNetworkAttempts += 1; throw new Error("V5_OFFLINE_VERIFIER_NETWORK_DENIED"); }) as typeof fetch;
  try {
    const ledgerBefore = readFileSync(ledgerPath);
    const protocolBytes = readFileSync(path.join(here, "protocol-v5.json"));
    const protocol = validateProtocolV5(JSON.parse(protocolBytes.toString("utf8")) as unknown);
    ...
    const independentCompilerRows = [...independentCompilerSources, ...independentCompilerDynamicSources, ...independentCompilerData]
      .sort((left, right) => left.path < right.path ? -1 : left.path > right.path ? 1 : 0);
    assert.equal(independentCompilerSources.length, 195, "independent AST compiler graph cardinality drifted");
    ...
    const isolatedCompiler = runIsolatedCompilerV5();
    assert.equal(isolatedCompiler.compilerClosureBytes, compilerClosureBytes.toString("utf8"), "isolated compiler closure bytes differ");
    assert.equal(isolatedCompiler.privateBytes, privateBytes.toString("utf8"), "isolated private exact wire differs");
    assert.equal(isolatedCompiler.publicBytes, publicBytes.toString("utf8"), "isolated public exact wire differs");
    ...
    assertExactRows(storedCompilerClosure.dynamicSourceInputs, independentCompilerDynamicSources, "compiler dynamic SOURCE_PATHS closure");
    ...
    assert.equal(storedCompilerClosure.exactSourceFileSetAndBytesSha256, sha256(stableJsonV5(independentCompilerSources)));
    assert.equal(storedCompilerClosure.exactDynamicSourceSetAndBytesSha256, sha256(stableJsonV5(independentCompilerDynamicSources)));
    assert.equal(storedCompilerClosure.exactDeclaredDataSetAndBytesSha256, sha256(stableJsonV5(independentCompilerData)));
    assert.equal(storedCompilerClosure.exactCompilerClosureSetAndBytesSha256, sha256(stableJsonV5(independentCompilerRows)));
    const compilerCore = { ...storedCompilerClosure };
    delete compilerCore.compilerClosureSemanticSha256;
    assert.equal(storedCompilerClosure.compilerClosureSemanticSha256, sha256(stableJsonV5(compilerCore)));
    assert.equal(storedCompilerClosure.compilerClosureSemanticSha256, protocol.compilerClosureContract.semanticSha256);
    const externalInputs = storedCompilerClosure.externalInputContract as Record<string, unknown>;
    const gitInput = externalInputs.git as Record<string, unknown>;
    const gitStdout = run("git", ["rev-parse", "HEAD"]);
    assert.equal(gitInput.stdoutUtf8Bytes, Buffer.byteLength(gitStdout, "utf8"));
    assert.equal(gitInput.stdoutSha256, sha256(gitStdout));
    assert.equal(gitInput.canonicalHead, gitStdout.trim());
    const environmentInput = externalInputs.environment as Record<string, unknown>;
    const expectedEnvironmentRows = [
      ...COMPILER_OS_ENV_NAMES_V5.flatMap((name) => {
        const value = process.env[name];
        return typeof value === "string" && value.length > 0
          ? [{ name, kind: "minimal_os", valueUtf8Bytes: Buffer.byteLength(value, "utf8"), valueSha256: sha256(value) }]
          : [];
      }),
      ...Object.entries(COMPILER_EXPLICIT_ENV_V5).map(([name, value]) => ({
        name,
    ...
    assertExactRows(environmentInput.rows, expectedEnvironmentRows, "isolated compiler exact environment rows");
    ...
    assert.equal(externalInputs.exactExternalInputsSha256, sha256(stableJsonV5({
      git: gitInput,
    ...
    assert.equal(protocol.compilerClosureContract.exactDynamicSourceInputs, independentCompilerDynamicSources.length);
    ...
    const fileSystemContract = storedCompilerClosure.fileSystemInputContract as Record<string, unknown>;
    const expectedContentSites = fsCalls
      .filter((row) => row.operation === "readFileSync")
      .map((row) => ({ path: row.path, line: row.line, character: row.character, callee: row.operation, argument: row.argument }));
    const expectedMetadataSites = fsCalls
      .filter((row) => ["existsSync", "realpathSync", "statSync"].includes(row.operation))
      .map((row) => ({ path: row.path, line: row.line, character: row.character, callee: row.operation, argument: row.argument }));
    assertExactRows(fileSystemContract.exactContentInputCallSites, expectedContentSites, "compiler content-input call sites");
    assertExactRows(fileSystemContract.exactMetadataInputCallSites, expectedMetadataSites, "compiler metadata-input call sites");
    assert.equal(fileSystemContract.repositoryByteInputPartitionsDisjointAndExhaustive, true);
    ...
    const evaluateLiteralPathExpression = (expression: ts.Expression, stack: readonly string[] = []): string => {
      const current = unwrapExpression(expression);
      if (ts.isIdentifier(current)) {
        assert(!stack.includes(current.text), `v2 path declaration cycle: ${[...stack, current.text].join(" -> ")}`);
        return evaluateLiteralPathExpression(variableInitializer(current.text), [...stack, current.text]);
      }
      assert(ts.isCallExpression(current), "v2 declared path must be a literal call/identifier expression");
      if (ts.isIdentifier(current.expression) && current.expression.text === "fileURLToPath") {
        assert.equal(current.arguments.length, 1, "fileURLToPath must have one import.meta.url argument");
        const argument = current.arguments[0]!;
        assert(ts.isPropertyAccessExpression(argument) && argument.name.text === "url" &&
          ts.isMetaProperty(argument.expression) && argument.expression.keywordToken === ts.SyntaxKind.ImportKeyword &&
          argument.expression.name.text === "meta", "v2 here must derive only from import.meta.url");
        return v2CompilerAbsolute;
      }
      assert(ts.isPropertyAccessExpression(current.expression) &&
        ts.isIdentifier(current.expression.expression) && current.expression.expression.text === "path",
      "v2 declared path call must use node:path");
      const method = current.expression.name.text;
      if (method === "dirname") {
        assert.equal(current.arguments.length, 1, "path.dirname must have one argument");
        return path.dirname(evaluateLiteralPathExpression(current.arguments[0]!, stack));
      }
      assert(method === "join" || method === "resolve", `v2 declared path uses unsupported path.${method}`);
      assert(current.arguments.length >= 2, `path.${method} requires one base and literal tail`);
      const base = evaluateLiteralPathExpression(current.arguments[0]!, stack);
      const tails = current.arguments.slice(1).map((argument) => {
        assert(ts.isStringLiteralLike(argument), `path.${method} tail must be a string literal`);
        assert(!path.isAbsolute(argument.text) && !argument.text.includes("\0"),
          `path.${method} tail must be a relative non-NUL literal`);
        return argument.text;
      });
      return method === "join" ? path.join(base, ...tails) : path.resolve(base, ...tails);
    };
    const pathsInitializer = unwrapExpression(variableInitializer("paths"));
    assert(ts.isObjectLiteralExpression(pathsInitializer), "v2 paths must remain an object literal");
    ...
    for (const property of pathsInitializer.properties) {
      ...
      assert(!pathProperties.has(property.name.text), `v2 paths duplicates ${property.name.text}`);
      pathProperties.set(property.name.text, property);
    }
    const expectedDeclaredPaths = new Map<string, string>([
      ["protocolV2", "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v2/protocol-v2.json"],
      ["originalProtocolV1", "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v1/protocol.json"],
      ["corpusPublic", "experiments/question-quality-20260715/corpus/original-connectivity-pilot-v1/source-public.json"],
      ["corpusPrivate", "experiments/question-quality-20260715/corpus/original-connectivity-pilot-v1/private/pilot-source.private.json"],
    ]);
    ...
    for (const [name, expectedRelative] of expectedDeclaredPaths) {
      ...
      const absolute = path.resolve(evaluateLiteralPathExpression(property.initializer));
      const relative = slash(path.relative(repoRoot, absolute));
      assert(relative && !relative.startsWith("../") && !path.isAbsolute(relative),
        `v2 declared path ${name} escaped repository root`);
      assert.equal(relative, expectedRelative, `v2 declared path ${name} AST resolution differs`);
      resolvedDeclaredPaths.push(relative);
    }
    assert.equal(new Set(resolvedDeclaredPaths).size, resolvedDeclaredPaths.length,
      "v2 declared data paths contain a duplicate");
    assert.deepEqual([...resolvedDeclaredPaths].sort(), [...compilerDataInputs].sort(),
      "v2 declared data AST paths differ from compiler closure data inputs");

    const sourcePathsInitializer = unwrapExpression(variableInitializer("SOURCE_PATHS"));
    assert(ts.isArrayLiteralExpression(sourcePathsInitializer), "v2 SOURCE_PATHS must remain an array literal");
    const sourcePathRows = sourcePathsInitializer.elements.map((element) => {
      assert(ts.isStringLiteralLike(element), "v2 SOURCE_PATHS rows must all be string literals");
      assert(!path.isAbsolute(element.text) && !element.text.includes("\0"),
    ...
    assert.equal(new Set(sourcePathRows).size, sourcePathRows.length, "v2 SOURCE_PATHS contains a duplicate");
    for (const relative of compilerDynamicSourceInputs) {
      assert(sourcePathRows.includes(relative), `v2 SOURCE_PATHS AST lost dynamic row ${relative}`);
    }
    ...
    assert(!Object.hasOwn(privateArtifact, "productionSourceClosure"), "v5 must not retain the inherited 32-row field as authority");
    assertExactRows(privateArtifact.productionCompilerClosure, independentCompilerRows, "private exact-wire compiler closure");
    assert.equal(privateArtifact.productionCompilerClosureSha256, sha256(stableJsonV5(independentCompilerRows)));
    ...
    const frozenV4Wire = [
      { bytes: 53_354, hash: "5c68b632eff4f3f0e963b13c3697b41ebaf7772a9088836da54ba0f027ee2003" },
      { bytes: 24_943, hash: "aed07356a5f580be158490ef12d8cf5555a7ea910a5ccc397e53356fa295e643" },
    ] as const;
    for (const [index, row] of exactRows.entries()) {
      const bodyText = row.bodyText;
      assert.equal(typeof bodyText, "string");
      assert.equal(row.bodyUtf8Bytes, frozenV4Wire[index]!.bytes, `wire ${index + 1} bytes differ from v4`);
      assert.equal(row.bodySha256, frozenV4Wire[index]!.hash, `wire ${index + 1} hash differs from v4`);
      ...
      assert.equal(sha256(stableJsonV5(schema)), row.schemaSha256);
    ...
    assert.throws(() => assertExactRows(independentCompilerRows.slice(0, -1), independentCompilerRows, "v5 one-row deletion regression"), /exact set\/bytes\/hash equality/u);

    const storedClosure = JSON.parse(readFileSync(path.join(here, "live-closure-v5.json"), "utf8")) as ReturnType<typeof computeLiveClosureV5>;
    const computedClosure = computeLiveClosureV5();
    assert.equal(stableJsonV5(computedClosure), stableJsonV5(storedClosure), "exact live closure set/hash differs");
    ...
    assert.equal(independentLiveRows.length, 23);
    ...
    const storedExternalRuntimeFiles = storedClosure.externalRuntimeFiles as Array<Record<string, unknown>>;
    assert.equal(storedExternalRuntimeFiles.length, protocol.liveClosureContract.exactExpectedExternalFiles);
    assert.equal(storedExternalRuntimeFiles[0]!.path, realpathSync.native(process.execPath));
    assert.equal(storedExternalRuntimeFiles[0]!.bytes, statSync(process.execPath).size);
    assert.equal(storedExternalRuntimeFiles[0]!.sha256, sha256(readFileSync(process.execPath)));
    ...
    assert.equal(authorReport.liveClosureSha256, sha256(readFileSync(path.join(here, "live-closure-v5.json"))));
    ...
    assert.equal(authorReport.exactLiveClosureFiles, 23);
    const frozenContract = protocol.frozenRuntimeContract as Record<string, unknown>;
    const frozenArtifactBytes = readFileSync(path.join(repoRoot, String(frozenContract.artifactPath)));
    assert.equal(sha256(frozenArtifactBytes), frozenContract.artifactSha256);
    const frozenArtifact = validateFrozenRuntimeArtifactV5(JSON.parse(frozenArtifactBytes.toString("utf8")) as unknown);
    assert.equal(frozenArtifact.contentSha256, frozenContract.artifactContentSha256);
    assert.equal(frozenArtifact.bundleSetSha256, frozenContract.bundleSetSha256);
    assert.equal(frozenArtifact.nodeRuntime.nodeVersion, frozenContract.nodeVersion);
    assert.equal(frozenArtifact.nodeRuntime.executableSha256, frozenContract.nodeExecutableSha256);
    assertCurrentNodeRuntimeV5(frozenArtifact.nodeRuntime);
    const rebuiltFrozen = await buildFrozenRuntimeV5();
    assert.deepEqual(rebuiltFrozen.artifactBytes, frozenArtifactBytes, "frozen runtime artifact is not reproducible");
    for (const bundle of frozenArtifact.bundles) {
      const stored = readFileSync(path.join(repoRoot, bundle.path));
      assertFrozenBundleBytesV5(frozenArtifact, bundle.role, stored);
      assert.deepEqual(stored, rebuiltFrozen.bundleBytesByRole.get(bundle.role), `${bundle.role} bundle is not reproducible`);
      const mutated = Buffer.from(stored);
      mutated[Math.floor(mutated.length / 2)]! ^= 1;
      assert.throws(() => assertFrozenBundleBytesV5(frozenArtifact, bundle.role, mutated), /bundle bytes differ/u);
    }
    ...
    assert(frozenArtifact.externalRuntimeSpecifiers.every((specifier) => specifier.startsWith("node:")));
    const operatorBundle = readFileSync(path.join(repoRoot, frozenArtifact.bundles.find((row) => row.role === "OPERATOR")!.path), "utf8");
    assert.doesNotMatch(operatorBundle, /node_modules[\\/]tsx|tsx\/dist|tsxCli/u);
    assert.equal((authorReport.frozenRuntime as Record<string, unknown>).artifactSha256, frozenContract.artifactSha256);
    assert.deepEqual(Object.keys(authorReport.closedV4AuditFindings as Record<string, unknown>).sort(), [
      "V4-CLI-001", "V4-CLOSURE-004", "V4-COST-005", "V4-LEDGER-003", "V4-PARSER-002",
    ]);
    ...
    assert.notEqual(protocol.exactWireCommitment.privateArtifactSha256, v3Wire.privateArtifactSha256, "v5 retained the v3 private artifact hash");
    assert.notEqual(protocol.exactWireCommitment.publicArtifactSha256, v3Wire.publicArtifactSha256, "v5 retained the v3 public artifact hash");
    ...
    const buildOutputSecond = run(process.execPath, [tsxCli, path.join(here, "build-offline.mts"), "--check"]);
    assert.match(buildOutput, /V5_WRITE_NO_WRITE_PARITY_CONFIRMED/u);
    assert.equal(buildOutputSecond, buildOutput, "two full isolated builds produced different summaries");
    assert.equal(manifestBoundContentDigest(), manifestBoundBefore, "two full isolated builds changed manifest-bound bytes");
    ...
    const forbiddenMetadataOutput = path.join(here, "private/verifier-metadata-must-not-exist.json");
    assert.equal(existsSync(forbiddenMetadataOutput), false);
    runMustFail(
      process.execPath,
      [tsxCli, path.join(here, "capture-price-snapshot.mts"), `--output=${forbiddenMetadataOutput}`],
      /AUTHOR_FREEZE_PERMANENTLY_NO_DISPATCH_V5/u,
    );
    assert.equal(existsSync(forbiddenMetadataOutput), false);
    runMustFail(process.execPath, [tsxCli, path.join(here, "operator-wrapper.mts")], /AUTHOR_FREEZE_PERMANENTLY_NO_DISPATCH_V5/u);
    ...
    assert.deepEqual(Object.keys(runnerExports).sort(), ["runSealedConnectivityPilotV5"], "production runner exposed additional authority");
    await assert.rejects(
      () => runnerExports.runSealedConnectivityPilotV5({
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      exactCompilerDynamicSourceInputs: independentCompilerDynamicSources.length,
