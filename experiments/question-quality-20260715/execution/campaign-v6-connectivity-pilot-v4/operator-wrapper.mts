...
const { validateProtocolV4 } = protocolCoreExports;
...
export async function launchConnectivityPilotV4AfterFutureAuthorization(input: {
  runId: string;
  priceSnapshotPath: string;
}): Promise<number> {
  const protocol = validateProtocolV4(JSON.parse(readFileSync(protocolPath, "utf8")) as unknown);
  ...
  if (authorization.liveExecutionAuthorized !== true || authorization.hostileAuditPassed !== true ||
    ...
    throw new Error("v4 dispatch command is absent and live execution remains blocked");
  }
...
async function blockedEntrypoint(): Promise<void> {
  throw new Error("No dispatch command is present in the v4 author freeze; create a separately audited authorization version.");
}
