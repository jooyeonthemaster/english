import {
  ...
  type EndpointPriceV4,
  type JsonObject,
  type PublicPriceSnapshotV4,
} from "./protocol-core";

export interface RawPublicPriceInputsV4 {
...
function normalizeOverrides(endpoint: JsonObject, label: string): EndpointPriceV4["overrides"] {
...
function normalizeEndpoint(endpoint: JsonObject, label: string, chargeDimensions: Set<string>): EndpointPriceV4 {
...
export function buildPublicPriceSnapshotV4(input: RawPublicPriceInputsV4): PublicPriceSnapshotV4 {
  const requestedIds = modelIdsV4();
  ...
  const core = {
    schemaVersion: "question-quality-openrouter-public-price-snapshot-v4" as const,
  ...
  const snapshot = { ...core, contentSha256: sha256V4(stableJsonV4(core)) };
  return validatePriceSnapshotV4(snapshot);
}

export function priceEvidenceForModelV4(snapshot: PublicPriceSnapshotV4, modelId: string): {
