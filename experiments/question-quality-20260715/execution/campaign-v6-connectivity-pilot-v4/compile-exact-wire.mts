...
export const exactWireV4Paths = {
  privateArtifact: path.join(here, "private/exact-wire-v4.private.json"),
  publicArtifact: path.join(here, "offline-exact-wire-seal-v4.json"),
} as const;

const sha256 = (value: string | Uint8Array): string => createHash("sha256").update(value).digest("hex");

export async function compileConnectivityPilotExactWireV4() {
  // This inherited compiler is used only as an offline production request compiler.
  // Its rejected v2 live runner is never imported into the v4 execution closure.
  ...
  assert.equal(compilerClosure.sourceFiles.length, 192);
  assert.equal(compilerClosure.declaredDataInputs.length, 4);
  assert.equal(compilerClosure.files.length, 196);
  ...
  const privateCore = {
    schemaVersion: "question-quality-v6-connectivity-pilot-exact-wire-private-v4",
  ...
  const privateArtifact = {
    ...privateCore,
    privateSemanticSha256: sha256(stableJsonV4(privateCore)),
  };
  const privateBytes = `${JSON.stringify(privateArtifact, null, 2)}\n`;
  const publicCore = {
    schemaVersion: "question-quality-v6-connectivity-pilot-exact-wire-public-v4",
    status: "OFFLINE_PRODUCTION_CORE_EXACT_WIRE_LIVE_BLOCKED",
  ...
  const publicArtifact = { ...publicCore, publicSemanticSha256: sha256(stableJsonV4(publicCore)) };
...
async function main(): Promise<void> {
  const result = await compileConnectivityPilotExactWireV4();
