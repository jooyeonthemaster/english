...
const { buildPublicPriceSnapshotV4 } = priceCoreExports;
const { modelIdsV4 } = protocolCoreExports;
...
function assertPrivateOutput(filePath: string): string {
  ...
  if (!relative || relative.startsWith("..") || path.isAbsolute(relative) || path.extname(absolute) !== ".json") {
    throw new Error("price snapshot output must be a new JSON file under the v4 private directory");
  }
  return absolute;
}
...
async function main(): Promise<void> {
  ...
  if (!outputFlag) throw new Error("--output=<v4-private-json-path> is required");
