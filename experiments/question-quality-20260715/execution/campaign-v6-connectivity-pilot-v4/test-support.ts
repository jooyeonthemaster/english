...
export interface DeterministicLocalResponseV4 {
...
export interface DeterministicScenarioResultV4 {
  ...
  readonly evidence: readonly ParsedResponseEvidenceV4[];
}
 ...
 */
export function runDeterministicLocalScenarioV4(
  responses: readonly (DeterministicLocalResponseV4 | "FAIL_BEFORE_RESPONSE" | "UNKNOWN_AFTER_SEND")[],
): DeterministicScenarioResultV4 {
  const evidence: ParsedResponseEvidenceV4[] = [];
  let started = 0;
  let settled = 0;
  for (let index = 0; index < responses.length && index < 2; index += 1) {
    ...
    try {
      evidence.push(parseConnectivityResponseV4(response));
      settled += 1;
    } catch {
      settled += 1;
      return { terminal: "FAILED_TERMINAL", started, settled, successes: evidence.length, candidateOpportunities: started, physicalFetches: started, completions: started, evidence };
