...
export const LIVE_ENTRYPOINTS_V4 = [
  ...
  const visit = (node: ts.Node): void => {
    if ((ts.isImportDeclaration(node) || ts.isExportDeclaration(node)) && node.moduleSpecifier && ts.isStringLiteralLike(node.moduleSpecifier)) {
      ...
      if (node.expression.kind === ts.SyntaxKind.ImportKeyword) {
        ...
        if (name === "readImmutableRepoBytesV4" || name === "readImmutableRepoJsonV4") {
...
export function computeLiveClosureV4() {
  const queue = LIVE_ENTRYPOINTS_V4.map((entry) => canonicalRepoFile(path.join(repoRoot, entry)));
  ...
  const core = {
    schemaVersion: "question-quality-connectivity-pilot-live-closure-v4",
    algorithm: "TYPESCRIPT_AST_TRANSITIVE_IMPORTS_PLUS_DECLARED_RUNTIME_DATA_V1",
    entrypoints: [...LIVE_ENTRYPOINTS_V4],
    files,
    exactFileSetAndBytesSha256: SHA256(stableJsonV4(files)),
    runtimeInterfaces: {
      dynamicPrivateAttestedJsonReads: dynamicPrivateReads,
      dynamicPrivateAttestedJsonContract: "price snapshot must be under v4/private, strict schema-v4, self-content-hashed, and <=15 minutes old",
      ...
      privateOutputContract: "exclusive v4/private run directory; response and append-only journal never public",
    },
    completeness: {
  ...
  return { ...core, closureSemanticSha256: SHA256(stableJsonV4(core)) };
}
