...
export type PilotPlanV3 = "STANDARD" | "PREMIUM";
...
export interface ConnectivityPilotProtocolV3 extends JsonObject {
  schemaVersion: "question-quality-v6-connectivity-pilot-protocol-v3";
  artifactId: "campaign-v6-connectivity-pilot-v3";
...
export interface EndpointPriceV3 {
  provider: string;
  endpointName: string;
  tag: string;
...
export interface PublicPriceSnapshotV3 extends JsonObject {
  schemaVersion: "question-quality-openrouter-public-price-snapshot-v3";
  fetchedAt: string;
  sources: {
...
const MODELS: Record<PilotPlanV3, string> = {
  STANDARD: "google/gemini-3.5-flash",
  PREMIUM: "google/gemini-3.1-pro-preview",
};

export function stableValueV3(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(stableValueV3);
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value as JsonObject)
      .filter(([, child]) => child !== undefined)
      .sort(([left], [right]) => left < right ? -1 : left > right ? 1 : 0)
      .map(([key, child]) => [key, stableValueV3(child)]));
  }
  return value;
}

export function stableJsonV3(value: unknown): string {
  return JSON.stringify(stableValueV3(value));
}

export function sha256V3(value: string | Uint8Array): string {
...
export function conservativeCostV3(input: {
  bodyBytes: number;
...
function validateAssignments(value: unknown, protocol: JsonObject): AssignmentV3[] {
  ...
  const rows = value.map((candidate, index) => {
    ...
    const plan: PilotPlanV3 = index === 0 ? "STANDARD" : "PREMIUM";
    ...
    const expectedCost = conservativeCostV3({
    ...
    return row as unknown as AssignmentV3;
  });
  return rows;
}

export function validateProtocolV3(value: unknown): ConnectivityPilotProtocolV3 {
  const row = exactKeys(value, "protocol", [
    ...
    "globalResearchLedger", "liveClosureContract", "privatePersistence",
    "publicResultContract", "authorization", "authorFreezeActivity",
  ]);
  literal(row.schemaVersion, "question-quality-v6-connectivity-pilot-protocol-v3", "schemaVersion");
  literal(row.artifactId, "campaign-v6-connectivity-pilot-v3", "artifactId");
  literal(row.status, "OFFLINE_AUTHOR_FREEZE_LIVE_EXECUTION_BLOCKED_PENDING_INDEPENDENT_AUDIT", "status");

  const lineage = exactKeys(row.lineage, "lineage", ["preserves", "replacesExecutionPackage", "v2Unmodified", "v2AuditDisposition"]);
  literal(lineage.preserves, "campaign-v6-connectivity-pilot-v2", "lineage.preserves");
  ...
  literal(pricing.schemaVersion, "question-quality-openrouter-public-price-snapshot-v3", "pricingEvidenceContract.schemaVersion");
  ...
  literal(wire.privateArtifactPath, "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v3/private/exact-wire-v3.private.json", "exactWireCommitment.privateArtifactPath");
  literal(wire.publicArtifactPath, "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v3/offline-exact-wire-seal-v3.json", "exactWireCommitment.publicArtifactPath");
  ...
  const closure = exactKeys(row.liveClosureContract, "liveClosureContract", [
    ...
    "minimumCountChecksForbidden", "testSupportExcluded",
  ]);
  literal(closure.artifactPath, "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v3/live-closure-v3.json", "liveClosureContract.artifactPath");
  ...
  return row as unknown as ConnectivityPilotProtocolV3;
}

export function validatePriceSnapshotV3(value: unknown): PublicPriceSnapshotV3 {
  ...
  literal(row.schemaVersion, "question-quality-openrouter-public-price-snapshot-v3", "priceSnapshot.schemaVersion");
  ...
  literal(row.contentSha256, sha256V3(stableJsonV3(core)), "priceSnapshot.contentSha256");
  return row as unknown as PublicPriceSnapshotV3;
}

export function modelIdsV3(): readonly [string, string] {
