...
import {
  ...
  validateProtocolV3,
  type AssignmentV3,
  type ConnectivityPilotProtocolV3,
  type JsonObject,
  type PilotPlanV3,
} from "./protocol-core";
import { priceEvidenceForModelV3 } from "./price-snapshot-core";
import { parseConnectivityResponseV3, type ParsedResponseEvidenceV3 } from "./response-parser";
import {
  livePrivateRootV3,
  readImmutableRepoBytesV3,
  readImmutableRepoJsonV3,
  readPrivateAttestedJsonV3,
  withExclusiveRepoJsonTransactionV3,
} from "./live-io";

const directNetworkFetchV3 = globalThis.fetch.bind(globalThis);
const PROTOCOL_PATH = "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v3/protocol-v3.json";
const EXACT_WIRE_PATH = "experiments/question-quality-20260715/execution/campaign-v6-connectivity-pilot-v3/private/exact-wire-v3.private.json";
...
const GLOBAL_BATCH_ID = "campaign-v6-connectivity-pilot-v3-two-call";

interface ExactWireRowV3 {
  ordinal: 1 | 2;
  plan: PilotPlanV3;
  modelId: string;
  endpoint: string;
...
interface ExactWireArtifactV3 extends JsonObject {
...
interface GlobalLedgerV3 extends JsonObject {
...
interface JournalEventV3 extends JsonObject {
  sequence: number;
  at: string;
  event: string;
  plan: PilotPlanV3 | null;
  details: JsonObject;
  previousHash: string;
  eventHash: string;
}

interface PrivateRunStateV3 extends JsonObject {
  schemaVersion: "question-quality-connectivity-pilot-private-run-v3";
  runId: string;
  terminal: boolean;
  status: string;
...
export interface PublicConnectivityResultV3 {
  schemaVersion: "question-quality-v6-connectivity-pilot-public-result-v3";
...
function exactWire(): { protocol: ConnectivityPilotProtocolV3; artifact: ExactWireArtifactV3 } {
  const protocolBytes = readImmutableRepoBytesV3(PROTOCOL_PATH);
  const protocol = validateProtocolV3(JSON.parse(protocolBytes.toString("utf8")) as unknown);
  const artifactBytes = readImmutableRepoBytesV3(EXACT_WIRE_PATH);
  if (sha256V3(artifactBytes) !== protocol.exactWireCommitment.privateArtifactSha256) {
    throw new Error("private exact-wire artifact differs from protocol commitment");
  }
  const artifact = JSON.parse(artifactBytes.toString("utf8")) as ExactWireArtifactV3;
  if (artifact.schemaVersion !== "question-quality-v6-connectivity-pilot-exact-wire-private-v3" ||
  ...
  if (sha256V3(stableJsonV3(core)) !== artifact.privateSemanticSha256) throw new Error("private exact-wire semantic hash differs");
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (wire.ordinal !== assignment.ordinal || wire.plan !== assignment.plan || wire.modelId !== assignment.modelId ||
        wire.endpoint !== ENDPOINT || wire.bodySha256 !== sha256V3(wire.bodyText) ||
    ...
    if (body.model !== assignment.modelId || body.n !== undefined && body.n !== 1 || body.stream === true ||
        stableJsonV3(body.provider) !== stableJsonV3({
          order: ["google-vertex/global"], only: ["google-vertex/global"], allow_fallbacks: false,
          require_parameters: true, data_collection: "deny", zdr: true,
        }) || stableJsonV3(body.reasoning) !== stableJsonV3({ enabled: false, effort: "none", exclude: true })) {
...
function assertAuthorized(protocol: ConnectivityPilotProtocolV3): void {
  ...
  if (process.env[LIVE_CHILD_ENV] !== "1" || authorization.liveExecutionAuthorized !== true ||
    ...
    throw new Error("v3 live dispatch is not authorized");
  }
  const key = process.env.OPENROUTER_API_KEY;
  if (typeof key !== "string" || key.length < 8) throw new Error("live child lacks its sole OpenRouter credential");
}

function validateFreshPriceSnapshot(protocol: ConnectivityPilotProtocolV3, snapshotPath: string) {
  const snapshot = validatePriceSnapshotV3(readPrivateAttestedJsonV3(snapshotPath));
  ...
  for (const assignment of protocol.durableBounds.assignments) {
    const evidence = priceEvidenceForModelV3(snapshot, assignment.modelId);
    ...
    const recomputed = conservativeCostV3({
...
function globalLedger(value: unknown): GlobalLedgerV3 {
  const ledger = asObject(value, "global research ledger") as GlobalLedgerV3;
...
function reserveGlobalTwo(): void {
  withExclusiveRepoJsonTransactionV3({
    ...
    lockSuffix: "campaign-v6-connectivity-pilot-v3",
    mutate(current) {
      const ledger = globalLedger(current);
      if (ledger.usedFullQuestionCandidates !== 0 || ledger.reservedFullQuestionCandidates !== 0) {
        throw new Error("v3 author freeze binds the global ledger at exactly 0 used / 0 reserved; reseal required");
      }
      ...
      const next = {
        ...
        batches: [...ledger.batches, {
          ...
          outputTokens: 0,
          costUsd: 0,
          note: "v3 connectivity pilot: exactly Standard then Premium, single-shot, no replay",
        }],
      };
...
function createPrivateState(runId: string): { runRoot: string; state: PrivateRunStateV3; append: (event: string, plan: PilotPlanV3 | null, details: JsonObject) => void; persist: () => void } {
  if (!/^[a-z0-9][a-z0-9-]{7,80}$/u.test(runId)) throw new Error("runId is invalid");
  const runsRoot = path.join(livePrivateRootV3, "runs");
  ...
  let sequence = 0;
  const append = (event: string, plan: PilotPlanV3 | null, details: JsonObject) => {
    sequence += 1;
    const core = { sequence, at: new Date().toISOString(), event, plan, details, previousHash: state.journalHeadHash };
    const row: JournalEventV3 = { ...core, eventHash: sha256V3(stableJsonV3(core)) };
...
function publicResult(protocol: ConnectivityPilotProtocolV3, state: PrivateRunStateV3): PublicConnectivityResultV3 {
  const assignments = state.assignments;
  const evidence = assignments.map((row) => row.evidence).filter(Boolean) as ParsedResponseEvidenceV3[];
  ...
  const core = {
    schemaVersion: "question-quality-v6-connectivity-pilot-public-result-v3" as const,
  ...
  return { ...core, executionArtifactSha256: sha256V3(stableJsonV3({ ...core, journalHeadHash: state.journalHeadHash })) };
}
...
async function executeOne(input: {
  ...
  let rawText = "";
  try {
    const response = await directNetworkFetchV3(ENDPOINT, {
      method: "POST",
    ...
    const evidence = parseConnectivityResponseV3({
      rawText,
      requestedModel: assignment.modelId,
    ...
    run.append("ASSIGNMENT_SUCCESS_TERMINAL", assignment.plan, {
      providerRequestIdSha256: sha256V3(evidence.providerRequestId),
      servedModelSha256: sha256V3(evidence.servedModel),
      providerSha256: sha256V3(evidence.provider),
    ...
    assignmentState.failureMessageSha256 = sha256V3(error instanceof Error ? error.message : String(error));
    run.state.settledAssignments += 1;
...
export async function runSealedConnectivityPilotV3(input: {
  runId: string;
  priceSnapshotPath: string;
}): Promise<PublicConnectivityResultV3> {
  ...
  run.append("DUAL_RESERVATION_COMMITTED", null, {
    ...
    globalLedgerPathSha256: sha256V3(GLOBAL_LEDGER_PATH),
  });
  try {
    for (let index = 0; index < loaded.protocol.durableBounds.assignments.length; index += 1) {
      ...
      const price = priceEvidenceForModelV3(snapshot, assignment.modelId);
    ...
    const evidence = run.state.assignments.map((row) => row.evidence).filter(Boolean) as ParsedResponseEvidenceV3[];
    settleGlobal({
