...
This protocol is fixed before either reviewer opens passage text. Reviewer A and Reviewer B each inspect all 186 bound rows independently. Sampling, dividing rows between reviewers, replacement, top-up, discussing row outcomes, and reading the other reviewer's private or public artifacts are prohibited until both manifests are frozen.
...
Reconciliation occurs only after both manifests are frozen: any `EXCLUDE` excludes; otherwise any `DOMAIN_REVIEW` defers; only `PASS/PASS` is a dual-lens frame pass. At least 38 such passes are required. Review outcomes cannot trigger replacement or top-up within the frozen 186 rows; the 83 unbound supply rows are not reserves for this frame.

No model/API/network/database call, secret access, or full-question generation is permitted during source review.
