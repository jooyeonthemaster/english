// luna md-opt 팔 평가 패킷 빌더 (26-08-08) — mdopt.json 게이트 통과분.
// 산출: eval-mdopt/{solve-m.md,audit-m.md,key-m.json}
...
const v = JSON.parse(readFileSync(resolve(DIR, "mdopt.json"), "utf8")) as { rows: any[] };
const usable = v.rows.filter((r) => !r.error && r.parsed?.markedPassage && r.gateIssues.length === 0);
const shuffled = lcgShuffle(usable, 216);

const key: Record<string, any> = {};
let solveMd = `# 어법 문항 블라인드 풀이 패킷 M (${shuffled.length}문항)\n\n각 문항: 다음 글의 밑줄 친 부분(①~⑤, «» 구간) 중, 어법상 **틀린** 것을 고르시오.\n`;
let auditMd = `# 어법 문항 품질 감수 패킷 M (${shuffled.length}문항)\n`;
...
mkdirSync(resolve(DIR, "eval-mdopt"), { recursive: true });
writeFileSync(resolve(DIR, "eval-mdopt", "solve-m.md"), solveMd);
writeFileSync(resolve(DIR, "eval-mdopt", "audit-m.md"), auditMd);
writeFileSync(resolve(DIR, "eval-mdopt", "key-m.json"), JSON.stringify(key, null, 2));
console.log(`패킷 ${shuffled.length}문항`);
for (const [id, k] of Object.entries(key)) console.log(`${id} ${k.passageTitle.slice(0, 24)} ans=${k.answers.join(",")}`);
