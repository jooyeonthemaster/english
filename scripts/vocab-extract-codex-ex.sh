...
4. **이 문장에 구조 오독이나 유사철자 혼동 자리가 실제로 있는가?**
   문법 용어로 찾지 말고 **아래 실물 여덟 개와 같은 자리**를 찾아라. 전부 실제 기출에서 나온 것이다.

   ① 도치 — `Only when two or more people share an enormous amount can the ... take place`
      → only 를 뒤의 명사에 붙여 "~만"으로 읽는다. when절일 때에만 가능하다는 뜻이다. (lemma: only)
   ② 문두 명사를 명령문으로 — `Use of 'best before' labels has legally allowed institutional waste.`
      → 문두 Use 를 "사용하라"는 명령으로 읽어 뒤의 has 가 주어를 잃는다. (lemma: use, 명사)
   ③ 부정의 범위 — `they didn't want to grow too much`
      → "성장을 원하지 않았다"로 읽는다. 부정은 too much 에만 걸린다. (lemma: too)
   ④ 목적격보어 — `This makes us information blinded`
      → makes us 를 "우리를 만든다"로 읽고 뒤의 blinded 를 놓친다. (lemma: make)
   ⑤ 대동사 — `he did just that`
      → did 를 그냥 "했다"로 읽어 앞의 행동을 가리킨다는 것을 놓친다. (lemma: do)
   ⑥ 축약 관계절 — `73 percent of voters surveyed denied ...`
      → surveyed 를 유권자가 조사했다는 뜻으로 읽어 denied 의 주체를 놓친다. (lemma: survey)
   ⑦ 철자 혼동 — `The latter students suffered ...` / `their parents or carers`
      → latter 를 later 로, carer 를 career 로 본다. (lemma: latter / carer)
   ⑧ 외래어 오독 — `advertisers find new outlets` / `a physical disturbance`
      → 한국어 '아웃렛'(할인매장) · '피지컬'(신체)로 읽는다. 여기서는 판로 · 물리적인. (lemma: outlet / physical)

   **있으면** 잡고, **없으면 만들지 마라.** 위 여덟 유형은 예시일 뿐 목록이 아니다 —
   같은 종류의 자리를 문장에서 찾는 것이 목적이고, 억지로 여덟 개를 채우는 것이 아니다.
...
node "$ROOT/scripts/vocab-stamp-brief.mjs" "$ROOT/$OUT/$ID.json" \
     "$BRIEF_HASH" "codex-ex" "$(date -Iseconds)" 2>/dev/null
