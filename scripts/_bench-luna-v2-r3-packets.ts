// 3라운드(paired: luna-g3 vs g36-g3) 평가 패킷 빌더 — R1/R2 와 동일 규격.
...
async function main() {
  ...
  const gen = JSON.parse(readFileSync(resolve(DIR, "gen-r3.json"), "utf8"));
  ...
  const items = shuffled(pass, 26081403);
  const key: Record<string, any> = {};
  const solve: string[] = ["# 어법 문항 세트 R3 — 블라인드 풀이 패킷\n"];
  const audit: string[] = ["# 어법 문항 세트 R3 — 적대 감수 패킷\n"];
  ...
  writeFileSync(resolve(OUT, "key-r3.json"), JSON.stringify(key, null, 2));
  writeFileSync(resolve(OUT, "solve-g3.md"), solve.join(""));
  writeFileSync(resolve(OUT, "audit-g3.md"), audit.join(""));
  console.log(`R3 어법 ${items.length}문항 → eval-v2/(key-r3/solve-g3/audit-g3)`);
}
main().catch((e) => {
  console.error(e);
  process.exit(1);
