...
## 1.5 무엇을 담는 코퍼스인가 — 이 한 절이 아래 규칙 전부를 지배한다

고3 학생이 이 지문을 **시험장에서 처음 읽는다**고 상상하라.
그 학생이 **읽다가 멈추는 자리**가 이 코퍼스가 담아야 할 전부다. 멈춤은 세 모양으로 온다.

- **모르는 말** — 사전을 찾아야 하는 단어. 어떤 지문에도 반드시 몇 개는 있다.
- **아는데 안 맞는 말** — 아는 뜻을 넣었더니 문장이 안 서는 자리.
- **낱말은 다 아는데 안 풀리는 말** — 단어를 다 알아도 뭉치면 뜻이 따로 서는 자리.

⚠️ **첫째와 셋째를 놓치기 쉽다.** 둘째(다의어)는 눈에 잘 띄어서 그것만 잔뜩 담고 끝내기 쉽다.
그러면 학생이 실제로 막히는 자리의 절반이 빠진 단어장이 된다.

파일을 쓰기 전에 지문을 **한 번 더 통독하며** 자문하라 —
  "이 지문에서 학생이 **사전을 찾을 말**을 내가 다 담았는가?"
  "**낱말로 쪼개면 뜻이 사라지는 덩어리**를 다 담았는가?"
둘 중 하나라도 '아니다'면 지금 보태라. 이건 개수를 채우라는 말이 아니다 —
**셋 중 어느 한 모양도 통째로 비어 있으면 안 된다**는 말이다.
...
4. **이 문장에 구조 오독이나 유사철자 혼동 자리가 실제로 있는가?**
   (도치·부분부정·가주어·목적격보어·대동사·분사구문 / 비슷한 철자의 다른 단어.
    **있으면** 잡고, **없으면 만들지 마라.**)
...
# 【실측 사고 2026-08-01】 기본값이 비어 있으면 `~/.codex/config.toml` 이 이긴다(현재 sol/ultra).
#   러너를 재가동하면서 `VOCAB_CODEX_MODEL` 을 빠뜨렸더니 **검증하지 않은 sol 로 한 라운드가 돌았고**,
#   실행 시간이 8분→18분으로 늘어 처리량이 3분의 1이 됐다. 원인은 "환경변수를 기억해야 한다"는
#   운용 방식 자체다 — 기억에 의존하는 설정은 언젠가 반드시 빠진다.
#   → **검증된 값을 스크립트 기본값으로 박는다.** 환경변수는 실험할 때 덮어쓰는 용도로만 남긴다.
# 【실측 2026-08-02】 luna 를 기본으로 두었다가 **구조 함정을 6분의 1만 잡는 것**을 발견해 되돌렸다.
#   본선 실측(sol 3,182건 vs luna 400건, 학년 통제): 구조계열 함정 1.33 → 0.20/지문.
#   luna 를 쓴 이유는 싸고 빠르다는 것이었는데 **속도는 같았다**(같은 지문 3갈래 시험: 19.8 vs 19.5분).
#   프롬프트로 메우려는 시도(견본 8개 제시)도 중간 집계에서 효과가 없었다.
### 【확정 2026-08-02, 사용자 결정】 luna · xhigh · 광폭 브리프(§1.5)
# 7지문 짝지은 시험 결과(같은 지문에 네 갈래):
#                    항목    함정  구조계열  관용류  advanced   소요
#   luna·high·현행   40.14   6.00    0.14    6.43    1.86     20.9분
#   luna·ultra·현행  41.29   7.86    0.86    6.57    1.86     39.2분
#   luna·xhigh·광폭  39.86   5.86    0.57    6.43    3.14     24.2분  ← 채택
#   sol·high·현행    41.14   7.43    0.86    8.86    2.86     26.0분
#
#   · advanced 는 **effort 로 안 움직였고(1.86→1.86) 브리프로 뚫렸다**(3.14, sol 초과).
#   · 구조계열은 반대로 **effort 에 정직하다**(0.14→0.57→0.86). xhigh 는 sol 의 66%.
#   · 관용류는 셋 다 6.4~6.6 으로 꼼짝 안 했다 — 모델 능력으로 보이며 sol 의 73% 수준이다.
#   · sol 보다 빠르고(24.2 vs 26.0분) 코덱스 한도를 훨씬 덜 쓴다.
CODEX_MODEL="${VOCAB_CODEX_MODEL:-gpt-5.6-luna}"
CODEX_EFFORT="${VOCAB_CODEX_EFFORT:-xhigh}"
...
# 산출물에 **출처**를 찍는다 — 어느 브리프로 뽑혔는지.
# 【실측 2026-08-01】 이게 없어서 개정 효과를 mtime 으로 추정하다 두 번 틀렸다(SPEC §12.4·§12.5).
#   해시는 **치환 전 템플릿** 기준이라 지문 ID 와 무관하게 브리프 판본마다 고정이다.
#   에이전트에게 시키지 않는다 — 스크립트가 결정론적으로 찍으니 토큰도 오기도 0이다.
BRIEF_HASH="$(printf '%s' "$PROMPT_TEMPLATE" | sha1sum | cut -c1-12)"
node "$ROOT/scripts/vocab-stamp-brief.mjs" "$ROOT/$OUT/$ID.json" \
     "$BRIEF_HASH" "codex-v2" "$(date -Iseconds)" 2>/dev/null
