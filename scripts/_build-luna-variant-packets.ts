// luna 변형팔 평가 패킷 빌더 (26-08-08) — variants.json 게이트 통과분(med14k·xhigh30k).
// 본편 빌더와 동일 렌더 — 산출: eval-variants/{solve-v.md,audit-v.md,key-v.json}
import { readFileSync, writeFileSync, mkdirSync } from "fs";
import { resolve } from "path";

const DIR = "experiments/question-quality-20260715/luna-bench-20260808";
const ARMS = new Set(["med14k", "xhigh30k"]);
const CIRCLED = ["①", "②", "③", "④", "⑤", "⑥", "⑦", "⑧", "⑨", "⑩"];
...
function renderMarked(markedPassage: string, _marks: Mark[]): string {
...
const v = JSON.parse(readFileSync(resolve(DIR, "variants.json"), "utf8")) as { rows: any[] };
const usable = v.rows.filter(
  (r) => ARMS.has(r.arm) && !r.error && r.parsed?.markedPassage && r.gateIssues.length === 0,
);
const shuffled = lcgShuffle(usable, 808);

const key: Record<string, any> = {};
let solveMd = `# 어법 문항 블라인드 풀이 패킷 V (${shuffled.length}문항)\n\n각 문항: 다음 글의 밑줄 친 부분(①~⑤, «» 구간) 중, 어법상 **틀린** 것을 고르시오.\n`;
let auditMd = `# 어법 문항 품질 감수 패킷 V (${shuffled.length}문항)\n`;
...
shuffled.forEach((r, i) => {
  ...
  for (const a of p.answers) {
    const m = p.marks.find((x: Mark) => x.label === a);
...
mkdirSync(resolve(DIR, "eval-variants"), { recursive: true });
writeFileSync(resolve(DIR, "eval-variants", "solve-v.md"), solveMd);
writeFileSync(resolve(DIR, "eval-variants", "audit-v.md"), auditMd);
writeFileSync(resolve(DIR, "eval-variants", "key-v.json"), JSON.stringify(key, null, 2));
