...
2. **행동 게이트**(Playwright, 실제 클릭): 클래스 선택 → 전체 자료 → 지문 2개 체크 →
   「학습지 생성」 → 모달에 `[data-sheet-variant]` 4개 존재(직독직해 분석본 합류로
   3→4, 26-08-31 F3) · `[data-workbook-module]`
...
- 하단 CTA 명칭은 **「학습지 생성」 유지**(26-08-15 사용자 결정). 좌측 nav 의 동명 메뉴와
  ...
  집으므로 `button[title="기본·실전·파이널·직독직해 학습지를 만듭니다"]` 로 특정할 것
  (실사고 · title 자구는 직독직해 분석본 합류로 26-08-31 F3 에서 개정 — 구
  「기본 학습지·파이널 원페이지를 만듭니다」 셀렉터는 죽은 자구다).
